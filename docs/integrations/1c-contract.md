...
- **`order`** — заказ, по которому выпущен документ: `externalId` заказа в 1С (тот, что 1С сама отдаёт в секции 2) и номер заказа в кабинете. **`null`** — у документа без заказа (выпуск «без заказа», `У-145`).
