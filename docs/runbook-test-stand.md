...
3. `git reset --hard` → `npm ci` **только если сменился** `package-lock.json` → `prisma generate` → `npm run build` → `prisma migrate deploy`.
4. Успех → `kill` главных процессов обеих служб; systemd поднимет их с новой сборкой.
...
/home/aiproc/stands/lk_otsfera/scripts/stand/update-stand.sh   # обновить сейчас, не ждать
```
