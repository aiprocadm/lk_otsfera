...
|---|---|---|---|---|
| С-1 | **Гейты** | `npm run typecheck` · `npm run lint` · `npm run test:unit` · `npm run boundaries` · `npm run deadcode` · `npm run dup:check`; при доступном Docker — `npm run gate`. Любой красный — `❌` | 06.09.2026, `4b2b98ff` | ✅ — typecheck 0 · lint 0 · boundaries 0 (2569 модулей) · dup:check 2,61 % (466 клонов, без роста) · knip — мёртвых файлов 0, остаток долга прежний (5 экспортов / 19 типов / 1 дубль) · unit ✅ 1101 файл / 11 906 тестов (1 файл и 3 теста пропущены), 10,6 мин · integration ✅ 182 / 1386, 2,6 мин · `next build` ✅ (только штатное `bullmq`). `gate` по-прежнему невозможен: Docker нет |
| С-2 | **Drift-аудит ТЗ** | Полный проход `CLAUDE.md` §16 по всем `У-N` (закрытые программы тоже — они описывают ожидаемое поведение): проверять цепочку поведения, не наличие файла. Обновить [AUDIT.md](AUDIT.md) | 06.09.2026, `4b2b98ff` | ✅ — `src` с `73fab7ab` не менялся вовсе, дрейфу взяться неоткуда; поэтому вместо сверки по diff проверены **вживую цепочки требований с самой старой датой аудита** (01–04.09.2026, остальные 199 строк — 05.09): `У-145` (кнопка «Создать документ» приходит от страницы роли, сервер запрещает выпуск и без неё), `У-164` (истечение КП по `validUntil` — процессор `expire-proposals` зарегистрирован в реестре очередей и в расписании `docs.expireProposals.cron`), `У-165` (правило «отказ без причины нельзя» живёт в сервисе `rejectProposal`, а не в форме: адаптер намеренно не отсекает пустую причину, ошибка `reason_required`). Цепочки целы, править [AUDIT.md](AUDIT.md) нечего |
| С-3 | **Живой обход §0** | По реестру меню (`lib/navigation/cabinet.ts`) и реестру вкладок карточки: заголовок = пункт меню, подзаголовок в одну строку, главная кнопка или пустой экран с кнопкой, крошки на вложенных, ширина ≤ 390 px без горизонтального скролла, ошибки на русском. Образец процедуры — записи 20–21.08.2026 в [AUDIT.md](AUDIT.md) | 06.09.2026, `4b2b98ff` | ✅ — обход шести кабинетов (`screen-rules-check`, `mobile-shell-check`) в полном прогоне Playwright против свежего стенда `:3100` без замечаний; экраны с прошлого прогона не менялись |
| С-4 | **Доступ и изоляция** | Прогнать `security.role-access-matrix.guardrail`; для роутов и сервисов, появившихся с прошлого прогона, — ручные IDOR-пробы (чужая организация, чужая компания, чужой партнёр, `teamMode`, `canSee*`); убедиться, что у новых страниц кабинетов есть серверный `canSee*` (§4 `CLAUDE.md`) | 06.09.2026, `4b2b98ff` | ✅ — 22 security-файла (14 unit / 118 + 8 integration / 36 тестов) зелёные; новых роутов и страниц нет (0 изменённых `page.tsx`/`route.ts` с прошлого прогона); мутации `С-5` подтвердили живость ролевых стражей: «менеджер считается руководителем», «седьмая роль в модели», «админский раздел настроек открыт руководителю», «право на запись подменено правом на чтение» (реквизиты и слушатели) — каждая поймана |
| С-5 | **Мутация стражей** | По кругу: взять следующие 5–10 тестов-стражей (`*.guardrail.test.ts` и стражи из `AUDIT.md`), сломать инвариант, убедиться, что тест падает, вернуть файл. Молчащий страж — `❌`. Вести список проверенных стражей ниже, чтобы круг не начинался заново | 06.09.2026, `4b2b98ff` | ✅ 11 из 11 — стражи после `security.order-deal-visibility` до конца алфавита: `security.role-access-matrix`, `security.role-model-inventory`, `security.settings-matrix`, `server-actions.async-exports`, `services.graceful-degrade`, `services.no-test-only-modules`, `services.organizationCard.tab-caps`, `services.requisites.partner-org`, `services.students`, `shell.command-palette`, `ui.empty-states`, `ui.no-internal-jargon`, `worker.processor-coverage` — каждая поломка поймана (у `shell.command-palette` первая проба была вне инварианта, см. журнал). **Круг замкнут: все 66 стражей проверены мутацией**, следующий прогон начинает новый круг с первого по алфавиту |
| С-6 | **Молчаливые ошибки** | `grep`/AST по `src/**` (кроме тестов): пустые `catch {}` и `.catch(() => {})` без `log`; `as unknown as`; `!` без комментария-обоснования; `TODO`/`FIXME`/`HACK`; `take:` без пагинации и счётчика; `process.env.` вне списка `У-122`; `console.*`; `revalidatePath`, не покрывающий все экраны объекта; свежие дубли `jscpd` | 06.09.2026, `4b2b98ff` | ✅ — пустых `catch {}` 0; тихих `.catch(() => {})` 3 — те же, в allow-list; `TODO`/`FIXME`/`HACK` 0; `console.*` 0; `process.env` — весь под реестром `У-122` (страж `config.env-registry` зелёный); `as unknown as` 13 — те же; свежих дублей нет (466 без роста). Разобраны два пункта процедуры, до которых прошлые прогоны не доходили: все 38 `!` (non-null) прикрыты проверкой рядом, и покрытие `revalidatePath` — см. журнал (ℹ) |
| С-7 | **Документация ↔ код** | `.env.example` и `.env.production.example` против `process.env` в коде; `docs/integrations/1c-contract.md` против `schemas.ts`/`rest-wire.ts`/`mock-1c`; `docs/glossary.md` против строк меню и вкладок; `CHANGELOG.md` за последние PR; команды README против `package.json`; `docs/feature-flags-matrix.md` против `featureFlags.ts` | 06.09.2026, `4b2b98ff` | ✅ — `.env.*` под стражем `У-134`; все `npm run` из README и рабочих чеклистов есть в `package.json`; `1c-contract.md` привязан к `oneCSync.schemas.test`; CHANGELOG актуален (docs-only PR итогов прогонов туда не пишутся), версия 0.10.0 / `[Unreleased]`; стражи команд, флагов, глоссария и `.env` зелёные в полном unit |
| С-8 | **Производительность** | Поиск `findMany`/`findFirst`/`count` внутри циклов (N+1); новые `where`/`orderBy` без индекса в `schema.prisma`; списки без `take` и без пагинации; тесты дольше 5 с (`vitest --reporter=verbose`) | 06.09.2026, `4b2b98ff` | ✅ — запросов не добавилось (`src` не менялся); файлов unit дольше 5 с — 0 (самый долгий `services.exports-renderers.unit` 4,0 с), в integration дольше 5 с нет; `EnrollmentRequest` без индекса по `organizationId` — запись прогона №4 в силе, таблица мала |
| С-9 | **Зависимости** | `npm audit --omit=dev` и `npm audit`; открытые PR renovate; уязвимые версии. Патч/минор с уязвимостью — чинить; мажор — только спекой (`⚠`) | 06.09.2026, `4b2b98ff` | ✅ — `npm audit --omit=dev` = 0; `npm audit` = 6 — тот же dev-хвост (`vitest`, `vite`, `vite-node`, `@vitest/mocker`, `@vitest/coverage-v8`, `esbuild`), принятый риск `Р-26`; PR ботов нет |
| С-10 | **Визуальные эталоны** | При доступном стенде (`npm run dev` + seed): `npm run e2e:visual`, включая проекты `mobile-*`. Красные **не по картинке** — `❌`; устаревшие эталоны — пересъёмка с объяснением в PR, что именно сдвинулось и почему это ожидаемо | 06.09.2026, `4b2b98ff` | ✅ — полный прогон 480 проверок (13 проектов) на свежем стенде `:3100`: **275 выполнены, 205 пропущены штатно, падений 0**, диффов по картинке 0, 47 мин. Первый прогон подряд без единого падения — хотфикс №13 закрыл мигавший страж фокуса модалки; пересъёмка не понадобилась |
...
круг замкнут 06.09.2026 (прогон №6): все 66 стражей проверены мутацией,
следующий прогон начинает новый круг с первого по алфавиту.
