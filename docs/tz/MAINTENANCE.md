...
| 06.09.2026 | С-5 | Страж `navigation.page-title-matches-menu.guardrail` молчит: читает только буквальные `<h1>`, а 89 страниц разделов ставят заголовок через `<PageHeader title=…>` — заголовков он не находил и проходил вхолостую (мутация «Заказы» → «Заявки» в `manager/orders/page.tsx` не поймана) | ❌ дефект | ✅ хотфикс №9 — @@XLINK@@ (прогон №4; страж читает `title="…"`, `title={'…'}`, `title={sectionLabel('…')}` — 69 из 89 разделов, и падает, если прочитал меньше 50; 3 мутации пойманы) |
