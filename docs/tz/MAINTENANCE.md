...
| 06.09.2026 | С-6 | Список переписок «Сообщения» (`services/chat/threads.ts:91`, `listThreads`, `take: 50`, «No cursor pagination in v1») без счётчика и подписи — в пяти кабинетах | ❌ дефект | ✅ хотфикс №11 — @@XLINK@@ (прогон №4; `THREADS_CAP`, `count` по тому же `where` → `total`, `ListCapNotice` в `OrderThreadInbox`, страницы передают `total`; 6 мутаций пойманы) |
