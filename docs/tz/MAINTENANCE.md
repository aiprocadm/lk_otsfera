...
| 05.09.2026 | С-1 | `npm run deadcode` (knip) на `12c3078a`: новый мёртвый экспорт `BOARD_CAP` в `services/deals/board.ts:76` — след PR [#507](https://github.com/aiprocadm/lk_otsfera/pull/507): тест доски сделок проверял `take: 500` числом, а не константой (у воронки и задач константа импортирована тестом `boards.cap-and-order.unit`) | ❌ дефект | ✅ хотфикс №8 — @@H8LINK@@ (прогон №3; тест `services.deals.board` импортирует `BOARD_CAP`, проверяет `take` и само значение; knip снова 5 экспортов = известный долг; 3 мутации пойманы) |
