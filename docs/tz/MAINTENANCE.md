...
|---|---|---|---|---|
| С-1 | **Гейты** | `npm run typecheck` · `npm run lint` · `npm run test:unit` · `npm run boundaries` · `npm run deadcode` · `npm run dup:check`; при доступном Docker — `npm run gate`. Любой красный — `❌` | 06.09.2026, `73fab7ab` | ✅ — на `5a7eff90` (код текущего `main`; хотфикс №13 трогает только `.spec.ts`, в vitest не входит): typecheck 0 · lint 0 · boundaries 0 (2569 модулей) · dup:check 2,61 % (466 клонов, без роста) · knip — **мёртвых файлов 0** (был 1: `partner/orgHistory.ts` убран хотфиксом №12), остаток долга прежний (5 экспортов / 19 типов / 1 дубль) · unit ✅ 1101 файл / 11 906 тестов (1 файл и 3 теста пропущены), 11,1 мин · integration ✅ 182 / 1386, 2,5 мин · `next build` ✅ (только штатное `bullmq`). `gate` по-прежнему невозможен: Docker нет |
| С-2 | **Drift-аудит ТЗ** | Полный проход `CLAUDE.md` §16 по всем `У-N` (закрытые программы тоже — они описывают ожидаемое поведение): проверять цепочку поведения, не наличие файла. Обновить [AUDIT.md](AUDIT.md) | 06.09.2026, `73fab7ab` | ✅ — с `42b16151` в `src` менялись 4 файла, все — хотфикс №12: удалены `partner/orgComments.ts` и `partner/orgHistory.ts`, переписан `security.idor-comments.integration`, добавлен страж `services.no-test-only-modules`. Якорей `У-N` на удалённые файлы в [AUDIT.md](AUDIT.md) нет — партнёрская карточка описана через `У-96`/`У-102` и живёт в `manager/organizationCard.ts`, цепочка «страница → сервис → тест» стала короче на мёртвое звено. Историческая матрица `docs/audit/2026-07-27-client-visibility-matrix.md` упоминает удалённый сервис как запись о прошлом — правке не подлежит (§9.4) |
| С-3 | **Живой обход §0** | По реестру меню (`lib/navigation/cabinet.ts`) и реестру вкладок карточки: заголовок = пункт меню, подзаголовок в одну строку, главная кнопка или пустой экран с кнопкой, крошки на вложенных, ширина ≤ 390 px без горизонтального скролла, ошибки на русском. Образец процедуры — записи 20–21.08.2026 в [AUDIT.md](AUDIT.md) | 06.09.2026, `73fab7ab` | ✅ — обход шести кабинетов (`screen-rules-check`, `mobile-shell-check`) в полном прогоне Playwright против свежего стенда `:3100` на `5a7eff90` без замечаний: заголовки совпадают с меню, вкладки карточки на месте, каркас на 390 px цел. Экраны в этом прогоне не менялись вовсе (удалён только неиспользуемый серверный код) |
| С-4 | **Доступ и изоляция** | Прогнать `security.role-access-matrix.guardrail`; для роутов и сервисов, появившихся с прошлого прогона, — ручные IDOR-пробы (чужая организация, чужая компания, чужой партнёр, `teamMode`, `canSee*`); убедиться, что у новых страниц кабинетов есть серверный `canSee*` (§4 `CLAUDE.md`) | 06.09.2026, `73fab7ab` | ✅ — 22 security-файла (14 unit / 118 + 8 integration / 36 тестов) зелёные; новых роутов и страниц нет. Изоляция комментариев карточки после хотфикса №12 проверяется строже прежнего: тест бьёт в живой `getOrganizationCard` с партнёрской сессией и сверяет И список, И счётчик вкладки (`tabTotals.activity`) — мутации «скоуп по продавцу вместо организации» и «`count` по другому условию» пойманы |
| С-5 | **Мутация стражей** | По кругу: взять следующие 5–10 тестов-стражей (`*.guardrail.test.ts` и стражи из `AUDIT.md`), сломать инвариант, убедиться, что тест падает, вернуть файл. Молчащий страж — `❌`. Вести список проверенных стражей ниже, чтобы круг не начинался заново | 06.09.2026, `73fab7ab` | ✅ 10 из 10 — стражи после `notifications.rules`: `oneCSync.pushable-types`, `pages.subtitles`, `pii.capture-coverage`, `prisma.enum-terminal-last`, `prisma.migrations-plain-sql`, `security.client-visibility`, `security.document-download-name`, `security.document-read-fields`, `security.document-status`, `security.order-deal-visibility` — каждая поломка поймана; плюс мутации хотфиксов №12 (7) и №13 (3). Следующий круг — с `security.role-access-matrix` |
| С-6 | **Молчаливые ошибки** | `grep`/AST по `src/**` (кроме тестов): пустые `catch {}` и `.catch(() => {})` без `log`; `as unknown as`; `!` без комментария-обоснования; `TODO`/`FIXME`/`HACK`; `take:` без пагинации и счётчика; `process.env.` вне списка `У-122`; `console.*`; `revalidatePath`, не покрывающий все экраны объекта; свежие дубли `jscpd` | 06.09.2026, `73fab7ab` | ❌ 1 → хотфикс №12 [#514](https://github.com/aiprocadm/lk_otsfera/pull/514) (очередь прогона №4 закрыта). ℹ: списки фильтров (`admin/auditLog`, `admin/piiAccess`, `admin/users/queries` — `distinct` по 200) счётчика не требуют, это варианты для выпадающего списка, а не выдача; `tasks/board` (50 связанных задач), `syncSummary` (50), `import/rollback` (20) ограничены самой моделью; `partner/orgDocuments` отдаёт `total`. Пустых `catch {}` 0; тихих `.catch(() => {})` 3 — те же, в allow-list; `.catch(() => null)` при разборе тела — не глотание (разобрано в прогоне №4); `TODO`/`FIXME` 0; `console.*` 0; новых `process.env` нет; `as unknown as` 13 — те же |
| С-7 | **Документация ↔ код** | `.env.example` и `.env.production.example` против `process.env` в коде; `docs/integrations/1c-contract.md` против `schemas.ts`/`rest-wire.ts`/`mock-1c`; `docs/glossary.md` против строк меню и вкладок; `CHANGELOG.md` за последние PR; команды README против `package.json`; `docs/feature-flags-matrix.md` против `featureFlags.ts` | 06.09.2026, `73fab7ab` | ✅ — `.env.*` под стражем `У-134`, новых `process.env` нет; CHANGELOG за #514–#515 на месте, версия 0.10.0 / `[Unreleased]`; все `npm run` из README есть в `package.json`; стражи команд, флагов и глоссария зелёные в полном unit; на удалённые хотфиксом №12 сервисы действующая документация не ссылается |
| С-8 | **Производительность** | Поиск `findMany`/`findFirst`/`count` внутри циклов (N+1); новые `where`/`orderBy` без индекса в `schema.prisma`; списки без `take` и без пагинации; тесты дольше 5 с (`vitest --reporter=verbose`) | 06.09.2026, `73fab7ab` | ℹ — запросов не добавилось (прогон только удалял код); самый долгий файл unit — `services.exports-renderers.unit` 4,0 с (порог 5 с), в integration долгих нет; `EnrollmentRequest` без индекса по `organizationId` — запись прогона №4 в силе, таблица мала |
| С-9 | **Зависимости** | `npm audit --omit=dev` и `npm audit`; открытые PR renovate; уязвимые версии. Патч/минор с уязвимостью — чинить; мажор — только спекой (`⚠`) | 06.09.2026, `73fab7ab` | ✅ — `npm audit --omit=dev` = 0; `npm audit` = 6 — тот же dev-хвост (`vitest`, `vite`, `vite-node`, `@vitest/mocker`, `@vitest/coverage-v8`, `esbuild`), принятый риск `Р-26`; PR ботов нет |
| С-10 | **Визуальные эталоны** | При доступном стенде (`npm run dev` + seed): `npm run e2e:visual`, включая проекты `mobile-*`. Красные **не по картинке** — `❌`; устаревшие эталоны — пересъёмка с объяснением в PR, что именно сдвинулось и почему это ожидаемо | 06.09.2026, `73fab7ab` | ❌ 1 → хотфикс №13 [#515](https://github.com/aiprocadm/lk_otsfera/pull/515). Полный прогон 480 проверок (13 проектов) на свежем стенде `:3100`: 274 выполнены, 205 пропущены штатно, диффов по картинке 0, 41 мин; единственное падение — нестабильный страж фокуса модалки (второй прогон подряд, см. журнал), после хотфикса файл зелёный на всех проектах (16 тестов, 1 мин); пересъёмка не понадобилась |
...
следующий прогон начинает со стража после `security.order-deal-visibility`.
