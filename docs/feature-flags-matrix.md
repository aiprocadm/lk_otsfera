# Матрица фиче-флагов для прод-релиза (R0.1)

Источник истины по семантике — [src/lib/featureFlags.ts](../src/lib/featureFlags.ts) (33 флагов: 5 opt-out default-ON, 28 opt-in default-OFF). Env-переменная — `FEATURE_<UPPER_SNAKE>`. Точки чтения route-флагов — тройные (§5 CLAUDE.md): middleware (404) + навигация + page/route-гейт; поведенческие флаги (`max_channel`, `staff_2fa` и т.п.) читаются в своих точках, перечисленных в комментарии флага. Состав таблиц и класс каждого флага сверяет с кодом страж `src/__tests__/docs.feature-flags-matrix.test.ts` — новый флаг без строки здесь не пройдёт `test:unit`.

**Главный инвариант проверен (аудит 2026-07-10): ни одна незавершённая фича не «включится сама» — все сырые флаги opt-in.** Заглушечные адаптеры (Mango REST, IMAP) дополнительно защищены двойным предохранителем «флаг + креды/адаптер».

## Opt-out (default ON — зрелые, env не выставлять)

| Флаг | Назначение | Прод | Зависимости |
|---|---|---|---|
| `commission_pdf` | Скачивание PDF стейтмента | **on** | Redis+worker (генерация) |
| `commission_xlsx` | Скачивание XLSX стейтмента | **on** | Redis+worker |
| `pwa_installer` | PWA-подсказка установки | **on** | — |
| `pii_access_log` | Журнал доступа сотрудников к ПДн (§25.7): запись — recordPiiAccess, просмотр — /admin/pii-access (не гейтится) | **on** | Postgres. Выключение = пауза журнала = комплаенс-дыра; только на время инцидента |
| `document_generation` | Выпуск документов по заказу: панель «Документы по заказу» во всех трёх кабинетах сотрудников, форма выпуска, предпросмотр (поведенческий). `У-144`: переведён в opt-out — включён по умолчанию | **on**; `0` — только аварийно | БД |

## Opt-in (default OFF — включать явно `=1`)

| Флаг | Назначение | Рекомендация в прод | Зависимости / почему |
|---|---|---|---|
| `organization_cabinet` | Кабинет организации | **1** (по staged-rollout runbook) | — |
| `manager_cabinet` | Кабинет менеджера | **1** | — |
| `leader_cabinet` | Кабинет руководителя | **1**, строго вместе с `manager_cabinet=1` | предупреждение в navigation/cabinet.ts |
| `enrollment_requests` | Заявки на обучение (5 ролей) | **1** | — |
| `chat` | Team-chat partner/org + чат-секция менеджера | **1** (комментарии к заказам — вне флага) | — |
| `role_constructor` | G1: конструктор ролей (`/leader/roles`, `/admin/roles`) | **1** технически готов; включать по бизнес-решению | БД |
| `sales_funnel` | G2: воронка (`/leader/funnel`, `/manager/funnel`) | **1** технически готов; по бизнес-решению | БД |
| `leader_analytics` | M3: аналитика руководителя — план/факт продаж (`/leader/analytics`; route: middleware + меню + page-гейт) | **1** технически готов; по бизнес-решению | БД |
| `internal_tasks` | G3: задачи (`/manager/tasks`, `/leader/tasks`) | **1** технически готов; по бизнес-решению | БД |
| `notif_queue` | Доставка уведомлений через воркер (идемпотентность по jobId) | **1** (Redis+worker в прод-контуре есть) | `REDIS_URL` |
| `max_channel` | Канал уведомлений Max | **0**, пока владелец не подключил бота | `MAX_BOT_TOKEN`/`MAX_WEBHOOK_SECRET` |
| `whatsapp_channel` | Канал уведомлений WhatsApp (агрегатор) | **0**, пока не подключён агрегатор | `WHATSAPP_AGGREGATOR_*` |
| `inbound_messaging` | Приём сообщений из мессенджеров: «Входящие письма» `/manager/inbox`, «Мессенджеры» `/manager/messengers` (диалоги), вебхуки ботов, server-actions диалогов. С 12.09.2026 (спека мессенджеров, `Р-М-4`) — **поведенческий**, переключается из «Функций платформы», как `telephony_mango` | **0** на старте, включать из интерфейса, когда подключён хотя бы один бот. **email-канал** — IMAP-адаптер = заглушка: `INBOUND_EMAIL_ADAPTER` держать `fake`/unset, иначе DLQ-шум каждые 2 мин | секреты вебхуков (задаются из интерфейса) |
| `telephony_mango` | Телефония `/manager/calls` + вебхук Mango (поведенческий, `Р-24`: снят с middleware, переключается из интерфейса) | **0** до реализации боевого REST-адаптера записей (сейчас заглушка → recording-джобы уходили бы в DLQ). При включении fail-fast требует `MANGO_API_KEY`/`MANGO_API_SALT` | `MANGO_*`, IP-allowlist |
| `staff_2fa` | 2FA сотрудников: email-код при логине admin/manager/leader (поведенческий флаг; точки чтения — login/2fa-роуты + секция настроек) | **1** после мержа PR и проверки доставки писем (EMAIL_ENABLED) | Resend (`EMAIL_ENABLED`, `RESEND_API_KEY`) |
| `contacts` | Справочник контактов (этап 1 ТЗ 12.09.2026, `У-178`…`У-187`, этап закрыт 13.09.2026): разделы «Контакты» в `/manager`, `/leader`, `/admin`, карточка контакта с шестью вкладками, форма, каналы, объединение дублей, категория «Контакты» в глобальном поиске, server actions контактов и «Контакта заказа», вкладки «Контакты» и «Заметки» карточки организации (реестр `orgCardTabs`), блок «Контакт» карточки лида, «Контакт заказа» в карточке заказа трёх кабинетов ЦО, поле «Контакт» формы сделки, а также прежние триаж-действия на `/manager/inbox` и `/manager/calls`. **Поведенческий**: страницы `notFound`, пункт меню и вкладки скрыты, server actions → `forbidden`, блоки в чужих карточках не рендерятся; включается из «Функций платформы» без env | **0** до приёмки этапа 1 заказчиком; включать из интерфейса, когда команда готова разбирать дубли (они всплывут сразу: контакты давно заводятся автоматически из писем и звонков) | БД; для «Написать» — `inbound_messaging`; право `crm.contacts` в профилях доступа |
| `bitrix_migration` | Миграция из Битрикс24 (этап 2 ТЗ 12.09.2026, `У-188`…`У-203`): раздел хаба «Интеграции → Миграция из Битрикс24» (только admin), server actions пакетов, роуты `/api/admin/bitrix/*`, процессор очереди `bitrix.import` (пакет останавливается с причиной «миграция выключена»), расписание `bitrix.resync`. **Поведенческий**: раздел `notFound`, действия → `forbidden`, очередь не обрабатывает; включается из «Функций платформы» без env | **0** до готовности вебхука Битрикс24 у заказчика; включать из интерфейса перед первым пакетом | БД; `APP_ENCRYPTION_KEY` для секрета вебхука; воркер для очереди |
| `comm_center` | Коммуникационный центр v2 (этап 3 ТЗ 12.09.2026, `У-204`…`У-217`): разделы хаба «Шаблоны ответов» и «Интеграции → Сайт», приём заявок с формы на сайте. **Поведенческий**: разделы `notFound`, публичный приём отвечает тем же безликим отказом, что и при выключенном приёме. Раздел «Мессенджеры» и приём входящих под флагом НЕ ходят — они остаются на `inbound_messaging`, иначе выключение рвало бы уже работающую переписку | **0** до приёмки этапа заказчиком; включать из «Функций платформы» | БД; для заявок с сайта — выпущенный токен формы |
| `automation` | Правила автоматизации (этап 4 ТЗ 12.09.2026, `У-222`…`У-224`): раздел хаба «Автоматизация», журнал срабатываний, исполнение правил. **Поведенческий**: раздел `notFound`, а диспетчер событий при выключенном флаге выходит сразу — выключенный робот не спрашивает правила даже у базы. Правила, помеченные активными, при выключенном флаге НЕ срабатывают | **0** до приёмки этапа заказчиком; включать из «Функций платформы» | БД |
| `staff_chat` | M4: внутренний чат сотрудников — секции «Чат команды» на `/manager/messages` и `/admin/messages`, `/api/staff-chat/*`, бейдж непрочитанного (поведенческий) | **1** технически готов; по бизнес-решению | БД |
| `staff_calendar` | M5: календарь сотрудников (`/manager/calendar`, `/leader/calendar`; меню + page-гейты + server-actions, без middleware — прецедент `internal_tasks`) | **1** технически готов; по бизнес-решению | БД |
| `global_search` | M6: глобальный поиск сотрудников (`/manager/search`, `/leader/search`, `/admin/search`; меню + page-гейты + сервис `globalSearch`, без middleware) | **1** технически готов; по бизнес-решению | БД |
| `client_requests` | Этап 5 (Модуль 1): заявки клиентов (`/partner/requests`, `/organization/requests`, `/manager/requests`, `/leader/requests`, `/admin/requests`; route: middleware + меню + page/route-гейты) | **1** после мержа этапа 5 | БД |
| `certificates_registry` | Этап 3: реестры удостоверений клиентов (`/organization/certificates`, `/partner/certificates`, карточка сотрудника, KPI-карточки) | **1** после мержа этапа 3 | — |
| `deals_pipeline` | Этап 6: канбан сделок (`/manager/deals`, `/leader/deals`; `/partner/orders` (бывший `/partner/deals`) — другой домен, вне флага) | **1** после мержа этапа 6 | БД |
| `intake_inbox` | Этап 7: «Входящие в работу» (`/manager/intake`, `/leader/intake`, `/admin/intake`) | **1** после мержа этапа 7 PR-2 | БД |
| `cabinet_questions` | Этап 9: «Задать вопрос» в кабинетах партнёра и организации (поведенческий) | **1** после мержа этапа 9 PR-1 | БД |
| `settings_hub` | ТЗ 2026-08-04: единый хаб «Настройки» в кабинетах сотрудников — состав сайдбара + редиректы со старых маршрутов (поведенческий) | **1** после приёмки; снимается вместе с приёмкой | — |

## Как флипать в проде

`.env.production` → выставить значение → `docker compose -f docker-compose.prod.yml up -d` (пересоздаёт web+worker с новым env). Порядок включения кабинетов — [runbook-staged-rollout-cabinets.md](runbook-staged-rollout-cabinets.md), релизные шаги — [runbook-launch-deploy.md](runbook-launch-deploy.md) §3.3.

## Владелец и правила выпиливания (фаза 7)

**Владелец всех флагов** — владелец продукта (aiprocadm): он решает включение
в проде и даёт «да» на выпиливание. Реестр обновляется в том же PR, что и код
флага (чек-лист PR-шаблона).

Правила выпиливания:

- **Opt-out (default ON)** — кандидаты на выпиливание: если флаг ≥3 месяцев в
  проде включён и ни разу не выключался, точки чтения убираются, код остаётся
  безусловным (отдельный PR с пометкой, какой флаг умер). Исключение —
  `pii_access_log`: аварийный рычаг комплаенса, не выпиливать.
- **Opt-in (default OFF)** — выпиливаются только вместе с фичей либо
  конвертируются в opt-out после стабильного включения в проде (решение
  владельца); прецедент — удаление `one_c_sync`/`document_scan` 2026-06-11
  (не имели точек чтения).
- Флаг без единой точки чтения = мёртвый код — удаляется немедленно
  (ловится ревизией матрицы против featureFlags.ts).
