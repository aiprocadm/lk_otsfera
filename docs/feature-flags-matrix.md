...
| `contacts` | Справочник контактов (этап 1 ТЗ 12.09.2026, `У-178`…`У-181`): разделы «Контакты» в `/manager`, `/leader`, `/admin`, карточка контакта, форма, каналы, объединение дублей, категория «Контакты» в поиске, server actions, а также прежние триаж-действия на `/manager/inbox` и `/manager/calls`. **Поведенческий**: страницы `notFound`, пункт меню, server actions → `forbidden`; включается из «Функций платформы» без env | **0** до приёмки этапа 1; включать из интерфейса, когда команда готова разбирать дубли (они всплывут сразу: контакты давно заводятся автоматически из писем и звонков) | БД; для «Написать» — `inbound_messaging`; право `crm.contacts` в профилях доступа |
