...
| # | Находка | Где | Оценка |
|---|---|---|---|
| S2 | **76 из 86 роутов без Zod-валидации входа.** **Частично закрыто фазой 4:** обвязка `withAuth`/`parseJsonBody` + Zod заведены на ~30 роутов (admin, messages/staff-chat, manager/enrollments/client-requests, partner/finance). **Остаток («фаза 4b», ждёт решения):** роуты с `formData`/файлами (upload, вложения, экспорт), вебхуки интеграций (свой контракт подписи), и точки со своими схемами строже «формы» (`partner/team`, `portfolio/rate`, `comments`, `notifications`) — их трогать без нужды опаснее, чем оставить. | `src/app/api/**` | остаток ~2–3 дня, по решению |
...
|---|---|---|---|
| A1 | **Прямые Prisma-запросы мимо сервис-слоя.** **Частично закрыто фазой 3:** компоненты вычищены полностью (нашлось 5, не 2 — правило `components-no-db` в CI не даст вернуться). **Остаток (ждёт решения):** 19 роутов, 27 страниц, 17 server-actions — типовой паттерн «мелкий справочник для фильтра прямо в странице»; худшие точки `api/comments/route.ts` (CRUD в роуте), `server-actions/inbound.ts` (бизнес-процесс на 15+ запросов), `app/manager/orders/[id]/page.tsx`. | MAJOR (остаток) | 1–2 недели инкрементально |
| A2 | ~25 файлов в `src/lib/**` вне `services/` ходят в БД. **Закрыто решением фазы 3 (31.07.2026):** заказчик узаконил слоистую структуру — `lib` и есть нижний слой, инфраструктурные модули (auth, notifications, monitoring, pii) живут там законно. Правило `lib-no-upward` в dependency-cruiser охраняет направление; переносить нечего. | закрыто |
| A3 | **Механической защиты границ нет.** **Закрыто фазой 3 (31.07.2026, решение заказчика: слоистая структура узаконена):** dependency-cruiser в `verify`/CI — no-circular, lib↛app/components/server-actions, components↛db, worker↛UI, src↛mock-1c. Найденные 10 нарушений исправлены: 3 цикла типов (mango barrel, accessProfile↔jwt), 2 импорта типов вверх (welcomeActions, orgPageContext), 5 компонентов с прямыми prisma-запросами (вынесены в сервисы — закрывает и A1-компоненты). | закрыто | — |
| A4 | Barrel-политика неоднородна: где-то `index.ts` (ui, notifications), где-то нет. | MINOR | попутно |
| A5 | Worker-процессоры ходят в БД через параметр `db`. **Закрыто решением фазы 3:** паттерн узаконен (передача клиента параметром = тестируемость); правило `worker-no-ui` охраняет единственную реальную границу воркера. | закрыто |
...
| # | Находка | Severity |
|---|---|---|
| D1 | **Float-агрегация денег в KPI партнёра** (`partner/finance.ts`). **Закрыто фазой 5 (01.08.2026):** суммирование переведено на `Prisma.Decimal` (в number — один раз на выходе DTO), регресс-тест на копеечный дрейф добавлен. | закрыто |
...
| D4 | Механического запрета `findMany` по company-scoped моделям без фильтра нет (защита — соглашение + isolation-тесты). Фаза 5 (01.08.2026): решено НЕ вводить рантайм-extension/эвристический линт (риск ложных срабатываний и вмешательства в прод-поведение) — систематическое закрытие переносится в фазу 6 (IDOR-инварианты на каждый endpoint). Дрейф схемы от миграций теперь ловится в CI (`migrate diff --exit-code`). | → фаза 6 |
...
кабинетов, spec-first процесс. **Фаза 9 (04.08.2026) закрыла дыры:**
`docs/ARCHITECTURE.md` (карта слоёв/ролей/потоков, mermaid), `docs/adr/`
(9 ADR из §23 ТЗ и зафиксированных решений + правило ведения), README
«запуск за 10 минут» (шаги сверены с кодом; найдены и исправлены 4
неточности старого README), README в 5 ключевых модулях (services, auth,
api, worker, components), дрейф CLAUDE.md §7 (3 очереди) починен.
Фронтенд-проверки: секретов в client bundle нет (178 'use client'-файлов
без серверных env); eslint-правило на инлайн-hex осознанно отложено —
372 легаси-вхождения, массовая правка = визуальный риск без функциональной
пользы (решение spec frontend-foundation §6 подтверждено).
