...
| # | Находка | Где | Оценка |
|---|---|---|---|
| S1 | **`/api/duplicates/by-inn` ищет по всей базе без границы тенанта.** `findByInn` делает `where: { inn }` / `where: { clientInn: inn }` глобально, роут пускает **любую авторизованную роль** (только `getSession`, роль не проверяется). Организация или партнёр может перебором ИНН (короткий ключ, rate limit не спасает от медленного перебора) выяснять существование чужих организаций/лидов у других компаний. | `src/app/api/duplicates/by-inn/route.ts`, `src/lib/services/duplicates/findByInn.ts` | 0,5–1 день + тесты. **Внимание: сужение выдачи — это [BEHAVIOR CHANGE]**, нужно решение заказчика (кому этот поиск вообще нужен: похоже, только staff). |
...
| S3 | **`npm audit`: 6 high.** Главное — `xlsx` (импорт платёжек/зачислений): prototype pollution + ReDoS, официального фикса нет. Нужна замена (exceljs уже в проекте) или изоляция парсинга. | `package.json` | 1–2 дня (миграция импорта на exceljs) |
| S4 | **Нет gitleaks/secret-скана в CI.** Секретов в git сейчас нет (`.env` не трекается, трекаются только `*.example`), но защита от будущего отсутствует. | `.github/workflows/ci.yml` | 0,5 дня |
