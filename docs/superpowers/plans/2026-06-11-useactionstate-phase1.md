...
- [x] **T1. Хук `useFormAction`** — `src/lib/ui/useFormAction.ts` (`'use client'`):
...
- [x] **T2. Admin «edit»-тройка**: `user-edit-form`, `partner-edit-form`, `organization-edit-form` → `useFormAction`; локальные `translateError` → `errorMap`-литералы (дельты, которых нет в `errorMessageRu`).
- [x] **T3. Admin «create/invite»-пара**: `partner-create-form`, `user-invite-form` — success-state с `inviteUrl` через `data` из хука; `reset()` для «создать ещё».
- [x] **T4. Остаток**: `assign-order-manager-form` (admin), `manager-status-change-form` (manager) — паттерны с feedback-state; сохранить существующие ERROR_LABELS-тексты как `errorMap`.
- [x] **T5. Холистическое ревью + полный гейт** (typecheck / lint / test:unit / build) + close-out.
