...
- [x] `Company.taskOverdueEscalationDays Int @default(3)`
- [x] Тип `task_overdue`; расширение `sla-escalation.ts` на задачи: исполнителю в день просрочки (`overdueNotifiedAt`), руководителю на N-й день (`overdueEscalatedAt`), оба claim атомарным `updateMany` по `null`
- [x] Блок «Просроченные» на дашборде руководителя
- [x] Раздел хаба `catalogs.slaIntake` → «SLA», второй блок «SLA задач» (`В-4-2`, умолчание); глоссарий и крошки в том же PR
- [x] «Мой день» менеджера: диалоги `waiting_staff` (мои), КП с истекающим сроком, прогресс чек-листов, события календаря
- [x] Страж `tasks.overdue-claims` — два уведомления в разные дни, каждое ровно один раз (мутация: склеить поля)
