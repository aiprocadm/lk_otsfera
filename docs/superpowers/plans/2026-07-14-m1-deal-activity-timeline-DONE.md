...
- **Атрибуция по контактам** (резолвинг звонков/сообщений по номеру телефона контакта, `User.contactPhone`/справочник) → M2 (спека §5.2). Сейчас `fromInternal` в click-to-call форме — свободный текст-placeholder (env/settings-маппинг внутреннего номера сотрудника не реализован, полноценное поле `User.internalPhone` — кандидат M2).
