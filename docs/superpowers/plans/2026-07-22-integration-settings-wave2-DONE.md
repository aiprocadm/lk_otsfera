...
Live smoke (headless Chromium на dev-сервере): все 6 групп форм рендерятся;
сохранение Telegram-кредов пишет токен в БД шифрованным (`v1:…`), значение
не утекает в HTML (только «задан»); пустой секрет при повторном сохранении
не затирает сохранённый; невалидный порт IMAP даёт «Проверьте заполнение
полей»; бейдж статус-панели переходит в «Подключено» (после TTL кэша ≤30 с).
Тестовые строки из БД удалены после проверки.
