...
- [x] Удалить страницы `app/partner/leads/{page,[id]/page,new/page}.tsx`.
- [x] Удалить роуты `app/api/partner/leads/**` (5 файлов, включая вложения).
- [x] Удалить сервисы `services/partner/{leads,leadAttachments}.ts`.
- [x] Удалить компоненты: `leads-table`, `leads-card-list`, `lead-create-form`,
...
- [x] Убрать nav-пункт `/partner/leads` из `navigation/cabinet.ts`.
- [x] Убрать флаг `partner_leads` из `featureFlags.ts` и его префикс из
      `middleware.ts` (по образцу удаления `one_c_sync`/`document_scan`).
- [x] Данные (`Lead`, `LeadAttachment`) НЕ трогаем — с ними работают сотрудники.
- [x] Проверить, что `/partner/leads` и `/api/partner/leads/*` дают 404 для партнёра.

### 2. Матрица видимости (артефакт приёмки)

- [x] `docs/audit/2026-07-27-client-visibility-matrix.md`: строка на каждую
      клиентскую поверхность — эндпоинт · роль · поля наружу · вердикт
      (`ok` / `over-fetch` / `leak`).
- [x] Периметр (решение заказчика — включая общие роуты): API партнёра,
...
- [x] Каждый `leak` — чинится в этом же PR; каждый `over-fetch` — переводится
      на явный `select`.

### 3. Явные `select` вместо пост-фильтрации (§7 ТЗ)

- [x] Клиентские сервисы (`services/partner/**`, `services/organization/**`)
      выбирают ровно поля своего DTO; `include` без вложенного `select` не
      остаётся.
- [x] Перенести/переименовать чувствительную
...
- [x] Тест `security.client-services-select.guardrail.test.ts`: в
...
- [x] Удалить тесты удалённого кода (партнёрские лиды: страницы, API, сервисы,
      компоненты — ~14 файлов).
- [x] Актуализировать затронутые (`api.featureFlags.gating`, `notifications.href`,
      `components.bottom-tab-bar`, `kpi-grid`, `attention-list`, `events-feed`,
      `security.idor-lead` — оставить staff-часть).
- [x] Новый негативный тест: партнёр дёргает `/api/partner/leads*` → 404.

### 6. Отгрузка

- [x] `npm run typecheck` + `npm run lint` + `npm run test:unit` зелёные;
      integration по затронутым местам на живом Postgres.
- [x] CHANGELOG.md, STATUS.md, PR с `base: main`.
