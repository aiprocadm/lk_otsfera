...
- [x] A1. `schema.prisma`: enum `ClientRequestSource` (partner_cabinet |
...
- [x] B3. Вложения: сервис по образцу leadAttachments (MIME/size, скан-очередь,
  presigned download, infected → 410), роуты подателя и staff.
- [x] B4. `POST /api/client-requests` (+ GET список своих) — тонкие роуты;
...
- [x] C1. Сервис `clientRequests/triage.ts`: `takeInTriage` (submitted →
...
- [x] C2. Очередь `/manager/requests` (+ зеркала leader/admin): company-scope
  (C8-паттерн inbox), список + деталка с действиями C1.
- [x] C3. Новый сервис создания лида сотрудником (`manual`): только
  manager/leader/admin; форма в разделе лидов менеджера.

## D. Запрет партнёрских лидов (ФТ-1.5–1.7)

- [x] D1. `createLead` (partner) и `POST /api/partner/leads` → `forbidden`;
  guard-тест критерия приёмки (UI и API).
- [x] D2. `/partner/leads*` → redirect на `/partner/requests`; nav партнёра:
  «Заявки» → `/partner/requests`; nav менеджера: «Заявки» → «Лиды», новый
  пункт «Обращения клиентов» → `/manager/requests`.
- [x] D3. Флаг `partner_leads`: вывести из точек чтения партнёрского создания
  (разделы менеджера не трогаем); redirect живёт под `client_requests`.

## E. Тесты (порог 100%) и ворота

- [x] E1. submit: скоупы, валидация, транзакционность, вложения (unit +
  integration).
- [x] E2. triage: конвейер, convertToLead (source/sourceRequestId/converted),
  reject, C8-скоуп, уведомления.
- [x] E3. Запрет: партнёр UI/API → forbidden (guard), redirect, nav.
- [x] E4. Миграция Lead (partner_legacy, nullable partnerId) — integration.
- [x] E5. Страницы/формы/бейджи; обновить существующие lead-тесты.
