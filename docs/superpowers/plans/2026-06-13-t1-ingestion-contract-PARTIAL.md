...
- [ ] **Task 14 (F6) — DEFERRED, нужен продуктовый вопрос.** Не баг, а семантика: в дашборде руководителя `debt` = outstanding по billed-заказам, а `perManager.totalAmount/paidAmount` = суммы по **активным** заказам (разные метрики, намеренно не сводятся). «Унификация фильтра» изменит смысл колонки. Нужно решение владельца: что показывает колонка денег у руководителя — стоимость активной работы или billed-суммы.
