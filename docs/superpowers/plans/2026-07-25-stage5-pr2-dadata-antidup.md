...
- [x] A1. Клиентский компонент `src/components/party/party-autocomplete.tsx`
...
- [x] A2. Встраивание: `client-request-form.tsx` (поле «Название компании» —
...
- [x] B1. Сервис `src/lib/services/duplicates/findByInn.ts`:
...
- [x] B2. `GET /api/duplicates/by-inn?inn=` — тонкий staff-роут
  (canTriageClientRequests-гейт; rate-limit 30/мин).
- [x] B3. Плашка `inn-duplicate-hint.tsx` (клиентский, staff-формы): при вводе
...
- [x] B4. Встраивание плашки: триаж-очередь заявок (раскрытие заявки с ИНН —
...
- [x] C1. PartyAutocomplete: debounce/фетч/выбор/деградация/клавиатура (jsdom).
- [x] C2. findByInn: staff-only, нормализация, выдача org+leads; роут: гейты,
  rate-limit; guard-тест «клиентские роли → forbidden».
- [x] C3. Плашка: показ/ссылки/не блокирует; встраивания; guard-тест B4.
- [x] C4. `typecheck`/`lint`/`test:unit` зелёные; integration по затронутым
