...
- [x] карточка лида: блок «Контакт» (`resolveContactByChannel` по телефону и
      почте лида) + «Создать контакт из данных лида»
- [x] карточка заказа (три кабинета): «Контакт заказа» —
      `services/orders/primaryContact.ts` `setOrderPrimaryContact`, панель с
      выбором из контактов организации, аудит
- [x] сделка: «Контакт» (`Deal.contactId`) в форме сделки, проверка в
      `deals/crud.ts` (контакт компании, организация совпадает или пуста)
- [x] тесты сервисов, actions, компонентов и страниц; `CHANGELOG.md`
