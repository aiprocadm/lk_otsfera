...
**Дата:** 2026-06-11 · **Статус:** draft — есть открытые вопросы (§7), реализация после их закрытия.
...
## 7. Открытые вопросы (нужно решение до Фазы 4)

1. **Кластер 3 (10 fetch-форм):** (а) конвертировать их API-роуты в server-actions (унификация, но ломает паттерн «тонкий роут + сервис» для уже-эталонных роутов и теряет внешние API-консьюмеры, если появятся), или (б) сделать sibling-хук `useFetchSubmit` с тем же возвращаемым контрактом (две реализации, но роуты не трогаем). Лёгкая рекомендация — **(б)**: роуты auth/login и partner/leads менять рискованно, а единый возвращаемый контракт даёт ту же DRY-выгоду в компонентах.
2. **auth-формы** (`login`, `reset-password`) — мигрировать вообще? Они до-кабинетные, без Result-контракта на сервере; можно исключить из scope (оставить кластеру 3 только partner/manager-формы).
