...
| A3 | НДС включён (`COMMISSION_VAT_MODE` default `full`) | База **без НДС** по умолчанию |
...
> База партнёра за месяц = Σ(фактически полученные платежи без НДС за период по дате `paidAt`) − Σ(возвраты без НДС за период). Комиссия = база × ставка, действовавшая в периоде. Период — календарный месяц (1-е … последнее число) по дате поступления оплаты.
...
## 3. Денежные правила, требующие подтверждения владельца (ratify на гейте ревью спеки)

- **R1 (A3, edge).** Платёж **без заказа** и с `vatAmount = null` (НДС не выделен, заказа для ставки НДС нет): извлекаем **дефолтные 20 % НДС** (`amount / 1.2`). Консервативно — никогда не переплачивает партнёру. Альтернатива «считать сумму уже чистой» отвергнута как риск переплаты.
- **R2 (A2).** Отрицательный нетто-месяц (Σ возвратов > Σ платежей в одном периоде): выплата по ведомости **зажимается в 0** (`totalCommissionAmount = max(0, Σ)`), но отрицательные строки **сохраняются** в позициях для аудита. Следствие: при зажиме сумма строк ниже хранимого `totalCommissionAmount` — осознанное отклонение (выплата не уходит в минус). Перенос «минуса» в следующий период — это уже A6/SP-2, не здесь.
...
export type PaymentForCalc = {
  ...
  isRefund: boolean;
  orderVatIncluded: boolean | null;  // null = платёж без заказа
  orderVatRate: Prisma.Decimal | null;
...
На каждую строку:
- **База без НДС (A3):**
  1. `vatAmount != null` → `amount − vatAmount`.
  2. иначе есть заказ (`orderVatIncluded != null`):
     - `orderVatIncluded === true` → `amount / (1 + (orderVatRate ?? 0.2))`.
     - `orderVatIncluded === false` → `amount` (НДС нет).
  3. иначе платёж без заказа и `vatAmount = null` → `amount / 1.2` (R1).
...
2. **Выборка платежей (A1, A4):** `Payment` где `paidAt ∈ [periodFrom, periodTo]` И разрешённый-партнёр = `partnerId`, где разрешённый-партнёр = `order?.partnerId ?? organization.partnerId`. Prisma-`where`:
   ...
   ```
   Select узкий: id, amount, paidAt, vatAmount, isRefund, orderId, order{orderNumber, partnerId, vatIncluded, vatRate}, organization{name, partnerId}.
...
Рендер позиций: nullable номер заказа → «—» / «без заказа»; отрицательные строки возвратов отображаются со знаком минус. Заголовок базы — «База без НДС». Без структурных изменений документа.

### 5.7 A7 — документация

- Шапка `calculator.ts`/`statement.ts`: формула базы (платежи−возвраты, без НДС, ставка на дату, период по `paidAt`) со ссылкой §9.2; упоминание, что A6/§9.5 — в SP-2.
...
- A3: платёж с `vatAmount` (НДС 5 %) → база без НДС; заказ `vatIncluded` без `vatAmount` → дефолт по ставке заказа; R1 (без заказа, без vatAmount) → `/1.2`.
...
2. `calculator.ts` (тесты A1–A3, A5-math, R2 первыми).
...
- R1 / R2 — подтвердить на гейте ревью спеки (раздел 3).
- Судьба `Organization.partnerCommissionRate` и `partner/rateOverride.ts` (мёртвый код после исключения из формулы) — оставить до отдельного решения; не удаляем в SP-1.
