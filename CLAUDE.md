...
### Состояние на 12.09.2026 — введена программа «CRM для отдела продаж — замена Битрикс24», текущий этап 1
...
93 требования `У-177`…`У-269` в AUDIT.md (`У-177` ✅, остальные `⏳`).
**Текущий этап — 1 «Контакты и внутренние заметки»** (`У-178`…`У-187`,
[04_crm_core.md](docs/specs/04_crm_core.md), задачи Task 1…4 в
[15_claude_code_tasks.md](docs/specs/15_claude_code_tasks.md)); шаг — спека
этапа в `docs/superpowers/specs/` **не написана**, начинать с неё и
предъявить заказчику до кода. Открытые вопросы `В-Б-1`…`В-Б-6` — в
`STATUS.md`, ни один не блокирует этап 1. Режим сопровождения не остановлен:
хотфиксы §9.4 идут параллельно, полные прогоны `С-1`…`С-10` — при закрытии
этапов и по просьбе заказчика. `npm run tz:status` — три doc-стража
(`docs.tz-program`, `docs.live-links`, `docs.commands-exist`), обязан быть
зелёным в каждом PR программы. Актуальную картину всегда бери из
