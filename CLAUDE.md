...
Действующее ТЗ — [docs/tz/2026-08-04-tz-fix-1c-import.md](docs/tz/2026-08-04-tz-fix-1c-import.md) («Починка загрузки выгрузки из 1С (Excel)» от 04.08.2026). Прогресс — **только** в [docs/tz/STATUS.md](docs/tz/STATUS.md).

**Какое ТЗ действующее — решает `STATUS.md`, а не дата или номер в имени файла.** Этот абзац — снимок; при расхождении верь `STATUS.md`.

**В `docs/tz/` лежат три ТЗ — не перепутай:**

- **[ТЗ починки импорта 1С](docs/tz/2026-08-04-tz-fix-1c-import.md)** (04.08.2026) — **действующее**, работа идёт по нему. Точечное ТЗ на один дефект: организации из Excel-выгрузки 1С не создаются, экран молчит. Объём — **§3 «План работ (порядок PR)»**: 10 этапов, 44 требования `Т-N`. Все вопросы заказчику в нём **уже закрыты** (§0.1) — блокеров нет.
...
> **Три ловушки этой тройки документов.**
> 1. **v0.5 новее v1.0.** Линейки версий независимые; меньший номер не значит «устарело». Не «возвращайся» к v1.0.
> 2. **ТЗ починки импорта — не «v0.6» и не замена v0.5.** Это отдельный документ на один дефект, живущий параллельно. v0.5 не отменено — оно просто больше не даёт новых работ.
> 3. **Ссылки вида «§N ТЗ» в разных документах означают совершенно разное** (в v1.0 §9–§11 — про процесс и этапы; в v0.5 §9 — комиссия, §10 — статусы, §11 — настраиваемые поля; в ТЗ починки импорта §3 — план PR, §0.1 — решения владельца). Поэтому шаги ниже ссылаются на разделы **этого файла**, а не на номера параграфов ТЗ. Встретишь «§N ТЗ» в исторических документах — сверься, о каком документе речь.
...
**Объём работ берётся из действующего ТЗ, а не из головы.** Сейчас это §3 ТЗ починки импорта: требование существует, только если у него есть номер `Т-N`. Программы v0.5 и v1.0 отгружены — «улучшать» их по своей инициативе нельзя. Новый пункт появляется в STATUS.md только после явной просьбы заказчика и подтверждённой спеки.
...
### Состояние на 05.08.2026 — программа открыта

По действующему ТЗ починки импорта 1С **работа есть**: заведены 10 этапов по
таблице §3. Текущий — **этап 1 «Диагностика» (Т-1…Т-3), спеки нет**. Ближайший
шаг по фразе «продолжай по ТЗ» — **написать спеку этапа 1 и предъявить её
заказчику до кода**.

Разведка по коду (три ключевых якоря перепроверены против `main` 5ad6ec4d) и
список того, что ждём от заказчика, лежат в `STATUS.md` — начинай с них, а не с
чистого листа. Актуальную картину всегда бери из `STATUS.md`: этот абзац —
снимок на дату и может отстать.

**Три порядковые зависимости этой программы — не переставлять и не «делать
заодно».** Каждая уже описана в ТЗ, и каждая при нарушении даёт молчаливый
провал, а не ошибку сборки:

| Требование | Что сломается при нарушении |
|---|---|
| Этап 4 (партнёр необязателен) **до** этапа 5 (импорт организаций) | первый прогон даст 100% `skip: partner_not_found` — работу придётся переделывать |
| Этап 6 (привязка к компании) **до** этапа 7 (роли) | доступ руководителю выдан формально, но каждая новая организация отбивается как `skip: out_of_scope` (Т-26а) |
| Этап 8 (writer'ы возвращают результат) **до** этапа 9 (откат) | откат построить не на чем: writer'ы сейчас возвращают `void` |

**Ловушка самого ТЗ:** сноска под таблицей §3 называет writer'ов «PR 7», хотя в
таблице это PR 8, а PR 7 — роли. **Первична таблица** — по ней заведены этапы в
`STATUS.md`. Вопрос вынесен заказчику, текст ТЗ не правился.

### Программа закрыта — что делать по «продолжай по ТЗ»

Порядок на случай, когда все этапы `✅` и остатков нет. **Сейчас это не так** —
программа открыта (см. блок выше), так что этот блок пока не про тебя. Он
понадобится, когда закроется действующая программа; так уже было дважды:
28.07–29.07.2026 (между v1.0 и v0.5) и 30.07–04.08.2026 (между v0.5 и ТЗ
починки импорта).
