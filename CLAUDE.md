...
- **`.github/workflows/ci.yml` — единственный workflow** (добавлен PR-серией укрепления, 2026-07): серверное зеркало лестницы хуков. Локальный гейтинг Husky остаётся первой линией; CI страхует от `--no-verify`. Новые workflow не добавляй без обсуждения; шаги CI не должны дрейфовать от npm-скриптов хуков.
  **Проверяй, что он включён.** 13.08.2026 workflow отключили в настройках
  репозитория, и до 08.09.2026 ни один PR не проверялся сервером: отключённый
  workflow не падает — он молчит, а `gh pr checks` отвечает «no checks
  reported», как если бы прогон ещё не начался. Состояние смотреть так:
  `gh api repos/<owner>/<repo>/actions/workflows -q '.workflows[].state'`
  (`active` / `disabled_manually`), запустить вручную — `gh workflow run ci.yml`.
...
Действующее ТЗ — [docs/tz/2026-09-12-tz-crm-bitrix-replacement.md](docs/tz/2026-09-12-tz-crm-bitrix-replacement.md) («CRM для отдела продаж — замена Битрикс24» от 12.09.2026; это **тонкий индекс**, полный пакет из 17 файлов — [docs/specs/](docs/specs/README.md), решение `Р-Б-10`). Прогресс — **только** в [docs/tz/STATUS.md](docs/tz/STATUS.md). Реестр проверок режима сопровождения — [docs/tz/MAINTENANCE.md](docs/tz/MAINTENANCE.md); предыдущее ТЗ 21.08.2026 закрыто 05.09.2026.
...
**В `docs/tz/` лежат семь документов-ТЗ — не перепутай:**

- **[ТЗ: CRM для отдела продаж — замена Битрикс24](docs/tz/2026-09-12-tz-crm-bitrix-replacement.md)** (12.09.2026) — **действующее**, работа идёт по нему. Это **индекс**: одна строка на требование со ссылкой на файл пакета [docs/specs/](docs/specs/README.md) (17 файлов: аудит кода `00`, видение `01`, роли `02`, ИА `03`, предметные файлы `04`…`13` и `16`, дорожная карта `14`, задачи для Claude Code `15` — Task 0…49). Объём — **§4**: 93 требования `У-177`…`У-269` (нумерация продолжает сквозную линию), разложенные по **11 этапам (0–10)** в §5; тексты требований **не дублируются** — при расхождении индекса с пакетом верен пакет. Решения `Р-Б-1`…`Р-Б-12` — §2 (заказчик может отменить любое до подтверждения спеки этапа; ключевые: `Р-Б-4` контакты раньше миграции, `Р-Б-6` правила автоматизации слушают реестр уведомлений и не порождают триггеры, `Р-Б-8` внутренние заметки — `OrganizationNote`, `Comment` не трогаем, `Р-Б-9` новых ролей нет). §3.6 — реестр дефектов **`Д-41`, `Д-42`, `Д-43`, `Д-47`, `Д-49`** (пакет объявляет диапазон до `Д-49`, но `Д-44`/`Д-45`/`Д-46`/`Д-48` нигде не описаны — вопрос `В-Б-6`, до ответа таких дефектов нет). **§0 — четыре приёмочных критерия:** понятность, правило зеркала, «ничего не включается на сервере» и **«Битрикс24 выключен»** (две недели работы только в ЛК после финального импорта). **§9 — режим сопровождения продолжается параллельно** для хотфиксов §9.4.
- **[ТЗ единой логики кабинетов, документов для заказчика и интеграций из интерфейса](docs/tz/2026-08-21-tz-cabinets-documents-integrations.md)** (21.08.2026) — **закрыта 05.09.2026** (81 PR #401…#497, [close-out](docs/tz/2026-09-05-tz-cabinets-program-DONE.md)); её `У-78`…`У-176` остаются описанием ожидаемого поведения и сверяются в режиме сопровождения. Объём — **§4 «Требования»**: 99 требований `У-78`…`У-176` (нумерация продолжает линию прежнего ТЗ, чтобы `У-N` был уникален во всём репозитории), разложенные по **10 этапам (0–9)** в §5. Решения заказчика `Р-11`…`Р-14` и делегированные исполнителю `Р-15`…`Р-24` — в §2 (делегированные действуют по умолчанию, заказчик может отменить любое до подтверждения спеки этапа; `Р-23` уточняет sibling-pattern §4 этого файла — общий компонент для зеркальных экранов допустим как презентационный с view-моделью, данные и права даёт сервис роли; `Р-24` снимает один флаг `telephony_mango` с edge-гейта). §3 — доказательная база с якорями и **реестр дефектов `Д-1`…`Д-40`** (каждый привязан к `У-N`). **§0 — три приёмочных критерия:** понятность (как прежде), **правило зеркала** (один объект — одно название и одно место во всех кабинетах) и **«ничего не включается на сервере»**. **§9 — режим сопровождения** (см. ниже).
...
**Объём работ берётся из действующего ТЗ, а не из головы.** Сейчас это §4 индекса «CRM для отдела продаж — замена Битрикс24» и пакет `docs/specs/`: требование существует, только если у него есть номер `У-N` (`У-177`…`У-269`); дефект существует, только если он в реестре §3.6 индекса (`Д-41`, `Д-42`, `Д-43`, `Д-47`, `Д-49`) **и** привязан к `У-N`. Файл `docs/specs/15_claude_code_tasks.md` (Task 0…49) — порядок задач внутри этапа, а не разрешение пропустить спеку и подтверждение заказчика. Программы v1.0, v0.5, починки импорта, понятности и кабинетов/документов/интеграций отгружены — «улучшать» их по своей инициативе нельзя. Новый пункт появляется в STATUS.md только после явной просьбы заказчика и подтверждённой спеки. **Исключений два:** (а) расхождение уже отгруженного кода с требованием любого ТЗ — дефект, а не новый объём, чинится по §16 без отдельной просьбы; (б) находки режима сопровождения с вердиктом `❌` чинятся хотфиксами в границах §9.4 ТЗ.

**Порядковые ограничения действующего ТЗ (§5 индекса, нарушать нельзя):** этап 1 (контакты и заметки) до этапа 2 (миграция из Битрикс24) — переносить нужно во что-то (`Р-Б-4`); этап 1 до этапов 3 и 4 — карточка контакта и `linkedContactId` нужны диалогам в карточках и задачам из контакта; этап 3 до этапа 4 — триггер «диалог без ответа дольше SLA» требует статусов диалога (`У-207`); `У-212` (сервер канала `cabinet`) до `У-234` (экран «Переписка с менеджером»); этапы 3 и 4 до этапа 7 (метрики коммуникаций и просроченные задачи); этап 9 (очистка) — после утверждения списка заказчиком (`Р-Б-3`), удаление таблиц (`У-261`) — **отдельным PR** после dry-run «0 строк», остальные миграции программы аддитивны и обратимы; действия правил автоматизации не порождают триггеры (`Р-Б-6`) — закрепляется тестом до первого правила из коробки. Этап 5 можно вести параллельно этапу 2.
...
### Состояние на 15.09.2026 — программа «CRM для отдела продаж — замена Битрикс24», этапы 1–3 закрыты, текущий этап 4

Программа введена в действие 12.09.2026 PR ввода (этап 0, `У-177`): пакет
ТЗ — [docs/specs/](docs/specs/README.md) (17 файлов, аудит на `a35d2d22`),
действующее ТЗ для стража — индекс
[docs/tz/2026-09-12-tz-crm-bitrix-replacement.md](docs/tz/2026-09-12-tz-crm-bitrix-replacement.md),
93 требования `У-177`…`У-269` в AUDIT.md.

**Этап 1 «Контакты и внутренние заметки» закрыт 13.09.2026** (`У-178`…`У-187`;
спека #585, PR #586…#591; close-out
[2026-09-12-stage1-contacts-and-notes-DONE.md](docs/superpowers/plans/2026-09-12-stage1-contacts-and-notes-DONE.md));
флаг `contacts` выключен до приёмки. **Этап 2 «Миграция из Битрикс24» закрыт
13.09.2026** (`У-188`…`У-203`; спека #596, PR #597…#602; close-out
[2026-09-13-stage2-bitrix-migration-DONE.md](docs/superpowers/plans/2026-09-13-stage2-bitrix-migration-DONE.md));
флаг `bitrix_migration` выключен, еженедельный повтор на паузе, выключение
Битрикс24 — работа заказчика по [runbook](docs/integrations/bitrix24-migration.md).
**Этап 3 «Коммуникационный центр v2» закрыт 15.09.2026** (`У-204`…`У-217`;
спека #607, восемь PR #608…#616; close-out
[2026-09-14-stage3-communication-center-DONE.md](docs/superpowers/plans/2026-09-14-stage3-communication-center-DONE.md));
флаг `comm_center` выключен до приёмки, под ним только НОВОЕ этапа — раздел
«Мессенджеры» остаётся на `inbound_messaging`, иначе выключение рвало бы уже
работающую переписку. Открытые вопросы этапа `В-3-10`…`В-3-12` и требование
задать `CLAMAV_HOST` на проде — в close-out.

**Текущий этап — 4 «Задачи и автоматизация»** (`У-218`…`У-227` (одиннадцать требований; `У-228`…`У-233` — это этап 5 «Центр уведомлений»)); шаг — сверка
кода по §16 и спека этапа в `docs/superpowers/specs/` **не написана**,
предъявить заказчику до кода. Открытые вопросы `В-Б-1`…`В-Б-6` — в `STATUS.md`.
Режим сопровождения не остановлен: хотфиксы §9.4 идут параллельно, полный
прогон `С-1`…`С-10` №29 по случаю закрытия этапа 3 — отдельной работой со своим
лимитом в три хотфикса. CI GitHub Actions с 12.09.2026 не запускается (аккаунт
заблокирован по оплате) — гейты гоняются локально. `npm run tz:status` — три
doc-стража (`docs.tz-program`, `docs.live-links`, `docs.commands-exist`),
обязан быть зелёным в каждом PR программы. Актуальную картину всегда бери из
`STATUS.md`: этот абзац — снимок на дату.

### Состояние на 05.09.2026 — программа ТЗ кабинетов, документов и интеграций закрыта (история)
...
[MAINTENANCE.md](docs/tz/MAINTENANCE.md). Двадцать четыре полных прогона `С-1`…`С-10`
выполнены 05–11.09.2026, сорок два хотфикса (#498…#570) влиты, 08.09.2026
нарезан релиз 0.11.0 (#537) и возвращены к жизни серверные проверки CI,
выключенные в настройках репозитория с 13.08.2026 (#539…#541); три вопроса
...
Действует **целиком**, когда все этапы таблицы `STATUS.md` стоят в `✅` и открытых PR нет
(решение `Р-20` ТЗ 21.08.2026; подробно — его §9). Пока программа идёт, режим
**не выключен, а сужен**: хотфиксы `❌` в границах §9.4 — в любой момент, полные
прогоны `С-1`…`С-10` — при закрытии каждого этапа и по прямой просьбе заказчика
(§9 индекса действующего ТЗ). Прежний ответ «работы нет,
