...
0. **Сначала посмотри шапку `docs/tz/STATUS.md` — строку «Текущий этап».** Если там стоит «программа … завершена / остатков нет» (состояние на 28.07.2026), незакрытых этапов НЕТ и продолжать нечего. В этом случае действуй по блоку «Программа закрыта» ниже — **не выдумывай себе работу и не начинай новый объём самовольно**.
1. Иначе определи **текущий этап** (первый не-`✅`) и на каком он шаге (спека / подтверждение / план / реализация / PR).
...
ТЗ v1.0 закрыто **28.07.2026**: все 12 этапов `✅`, остатков нет, итог — в
[close-out программы](docs/tz/2026-07-28-tz-program-DONE.md). Когда фраза
приходит в этом состоянии, порядок такой:
...
2. **Загляни в раздел «Задел на будущее» в `STATUS.md`.** Там перечислено то,
   что осознанно НЕ входило в ТЗ v1.0 (с пометкой, почему). Это **не
   обязательства**: брать оттуда задачу можно только по явной просьбе
   заказчика.
3. **Скажи заказчику прямо, что по ТЗ v1.0 работы нет**, и предложи выбор:
   новый объём (нужна новая версия ТЗ или отдельная спека по §8) либо задача
   из «Задела». Не начинай ни то, ни другое без ответа.
...
```bash
grep -c "✅ готов" docs/tz/STATUS.md      # сколько этапов закрыто
gh pr list --state open                   # незакрытые PR
```
