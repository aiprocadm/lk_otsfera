...
> **Изменится этапом 8 действующего ТЗ (`У-65`…`У-68`).** Значения флагов
> переезжают в таблицу `Setting` и становятся редактируемыми из
> `/admin/settings/system/feature-flags`; приоритет источников — **база → env →
> значение по умолчанию**, каждое переключение пишется в `AuditLog`. До мержа
> этапа 8 правило выше действует как есть: источник значения — env, страница
> read-only. Не начинай перевод раньше очереди (§14).
...
Действующее ТЗ — [docs/tz/2026-08-08-tz-usability-and-core-gaps.md](docs/tz/2026-08-08-tz-usability-and-core-gaps.md) («Понятность интерфейса и закрытие функциональных пробелов» от 08.08.2026). Прогресс — **только** в [docs/tz/STATUS.md](docs/tz/STATUS.md).

**Какое ТЗ действующее — решает `STATUS.md`, а не дата или номер в имени файла.** Этот абзац — снимок; при расхождении верь `STATUS.md`.

**Указатель на действующее ТЗ живёт в четырёх файлах и разъезжается именно при смене программы** — меняя программу, обнови все четыре: этот файл (§14), [docs/tz/STATUS.md](docs/tz/STATUS.md) (шапка и «Текущий этап»), [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) (карта документации) и [docs/tz/AUDIT.md](docs/tz/AUDIT.md) (реестр сверки). Согласованность держится механически: тест-страж [docs.tz-program.test.ts](src/__tests__/docs.tz-program.test.ts) падает, если хоть один из четырёх называет **другое** действующее ТЗ, если файл ТЗ пропал, если в `STATUS.md` нет «Текущего этапа» или если хоть одно требование `У-N` не разложено ни по одному этапу и не заведено в реестре.

**В `docs/tz/` лежат четыре ТЗ — не перепутай:**

- **[ТЗ понятности и функциональных пробелов](docs/tz/2026-08-08-tz-usability-and-core-gaps.md)** (08.08.2026) — **действующее**, работа идёт по нему. Объём — **§4 «Требования»**: 77 требований `У-N` (плюс `У-34а`), разложенные по **9 этапам** в §5 «План работ (порядок PR)». Вопросы заказчику закрыты (§8; решения `Р-1`…`Р-10` — в §2) — блокеров нет. **§0 — «пользователь должен легко ориентироваться в функционале» — это приёмочный критерий всего ТЗ**, а не пожелание к одному экрану (как применять — §15 этого файла).
...
> **Четыре ловушки этой четвёрки документов.**
...
> 3. **Ссылки вида «§N ТЗ» в разных документах означают совершенно разное** (в v1.0 §9–§11 — про процесс и этапы; в v0.5 §9 — комиссия, §10 — статусы, §11 — настраиваемые поля; в ТЗ починки импорта §3 — план PR, §0.1 — решения владельца; **в действующем ТЗ §2 — решения `Р-N`, §4 — требования `У-N`, §5 — план PR, §0 — приёмочный критерий понятности**). Поэтому шаги ниже ссылаются на разделы **этого файла**, а не на номера параграфов ТЗ. Встретишь «§N ТЗ» в исторических документах — сверься, о каком документе речь.
...
**Объём работ берётся из действующего ТЗ, а не из головы.** Сейчас это §4 ТЗ понятности: требование существует, только если у него есть номер `У-N`. Программы v1.0, v0.5 и починки импорта отгружены — «улучшать» их по своей инициативе нельзя. Новый пункт появляется в STATUS.md только после явной просьбы заказчика и подтверждённой спеки. **Исключение одно:** расхождение уже отгруженного кода с требованием действующего ТЗ — это дефект, а не новый объём; он чинится по §16 без отдельной просьбы.

**Порядковые ограничения действующего ТЗ (§5, нарушать нельзя):** этап 2 (единый шелл) до этапа 3 (мобильная версия) — иначе мобильную вёрстку придётся делать в пяти шеллах; этап 5 (справочник сотрудников) до этапа 6 (заявка «много слушателей × много обучений») — мастер выбирает сотрудников из справочника; `У-34` (бэкфилл направлений) и `У-36` (удаление поля с шапки) — **в разных PR**, иначе откатывать будет нечего.
...
### Состояние на 13.08.2026 — программа ТЗ понятности ЗАКРЫТА

**Все девять этапов `✅`, все 77 требований `У-N` (+ `У-34а`) отгружены**
(PR #333…#363), финальный аудит §16 дал 78 × `✅` и 0 × `❌`. Открытых PR нет.

Значит, **блок «Программа закрыта» ниже действует прямо сейчас**: по фразе
«продолжай по ТЗ» работы нет — скажи это заказчику и предложи выбор, а не
выдумывай себе объём. Остатки на его стороне (включить CI после обновления
лимитов, решение по красному гейту покрытия — долг этапа 5, перегенерация
эталонов `e2e:visual`, хвосты прежней программы) перечислены в шапке
`STATUS.md`; работой для агента они не являются, пока заказчик не попросит.

Актуальную картину всегда бери из `STATUS.md`: этот абзац — снимок на дату и
может отстать.

### Программа закрыта — что делать по «продолжай по ТЗ»

Порядок на случай, когда все этапы `✅` и остатков нет. **Сейчас это наш
случай** (см. снимок выше): программа ТЗ понятности закрыта 13.08.2026. Так уже
было трижды до этого: 28.07–29.07.2026 (между v1.0 и v0.5), 30.07–04.08.2026
(между v0.5 и ТЗ починки импорта) и 07.08–08.08.2026 (между починкой импорта и
ТЗ понятности) — каждый раз пауза заканчивалась новым ТЗ от заказчика.
