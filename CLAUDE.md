...
### Состояние на 07.08.2026 — программа ЗАКРЫТА

**Все 10 этапов ТЗ починки импорта 1С отгружены** (44 требования `Т-N`,
PR #310…#328, 05–07.08.2026). Открытых работ по ТЗ нет — по фразе «продолжай
по ТЗ» действуй по блоку «Программа закрыта» ниже. Остатки на стороне
заказчика (файл 1С для алиасов, флаги на проде, прогон
`npm run backfill:orphan-companies`) перечислены в шапке `STATUS.md`.
Актуальную картину всегда бери из `STATUS.md`: этот абзац — снимок на дату и
может отстать.

### Программа закрыта — что делать по «продолжай по ТЗ»

Порядок на случай, когда все этапы `✅` и остатков нет — **сейчас ровно этот
случай** (см. снимок выше). Так уже было трижды: 28.07–29.07.2026 (между v1.0
и v0.5), 30.07–04.08.2026 (между v0.5 и ТЗ починки импорта) и с 07.08.2026
(после закрытия ТЗ починки импорта).
