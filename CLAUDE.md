...
### Состояние на 08.08.2026 — идёт программа ТЗ понятности

**Заведена новая программа: 9 этапов, 77 требований `У-N`** (ТЗ от 08.08.2026).
Прежняя программа — починка импорта 1С — закрыта 07.08.2026 (PR #310…#328), её
блок «Программа закрыта» ниже **сейчас не применяется**: работа есть, текущий
этап — первый не-`✅` в `STATUS.md` (на дату записи — **этап 1, хотфикс
партнёрской ставки комиссии, `У-1…У-4`**). Остатки прежней программы на стороне
заказчика (файл 1С для алиасов, флаги на проде, прогон
`npm run backfill:orphan-companies`) перечислены в шапке `STATUS.md` и работам
по новому ТЗ не мешают. Актуальную картину всегда бери из `STATUS.md`: этот
абзац — снимок на дату и может отстать.

### Программа закрыта — что делать по «продолжай по ТЗ»

Порядок на случай, когда все этапы `✅` и остатков нет. **Сейчас это не наш
случай** (см. снимок выше) — блок держим для будущего. Так уже было трижды:
28.07–29.07.2026 (между v1.0 и v0.5), 30.07–04.08.2026 (между v0.5 и ТЗ
починки импорта) и 07.08–08.08.2026 (между починкой импорта и ТЗ понятности).
