...
- **Этап 6 ТЗ, PR-3 — аналитика сделок в план/факте (Модуль 4, ФТ-4.5).**
  Завершает этап 6. В план/факт руководителя (`/leader/analytics`) добавлена
  колонка «Выиграно сделок»: счётчик и сумма выигранных за месяц сделок
  (по `wonAt`, группировка по ответственному, строка «Без менеджера»);
  модель `SalesTarget` и факт по оплатам — без изменений.
