...
import { generateOrderDocument, type GenerateDocType, type GenerateResult } from '@/lib/services/documents/generate';
...
export type RequestRequisitesResult = { ok: true } | { ok: false; error: 'forbidden' | 'not_found' };
...
export async function requestRequisitesAction(fd: FormData): Promise<RequestRequisitesResult> {
  ...
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    select: { id: true, title: true, orderNumber: true, companyId: true, organizationId: true, managerId: true }
  });
  ...
  const missing = listMissingRequisites(company, organization).filter((m) => m.side === 'organization');
  try {
    ...
    log.warn('[documents/requestRequisites] notify failed', { orderId, error: (err as Error).message });
  }
  return { ok: true };
}
