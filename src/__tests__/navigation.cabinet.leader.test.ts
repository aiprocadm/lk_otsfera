...
describe('канон leader', () => {
  it('13 пунктов: сводка/команда/финансы/корректировки/заказы/организации/роли/воронка/задачи/обучение/сообщения/мои заказы/настройки', () => {
  ...
  it('пункты leader-меню без flag, кроме «Заявок», «Ролей», «Воронки» и «Задач» (свои opt-in флаги)', () => {
  ...
  it('navItemsFor("leader") без opt-in флагов скрывает «Заявки на обучение», «Роли», «Воронку» и «Задачи»', () => {
    ...
    expect(navItemsFor('leader')).toHaveLength(navByRole.leader.length - 4);
  });
