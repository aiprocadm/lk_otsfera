...
describe('OrderTimeline', () => {
  it('renders all events greyed-out (not passed) when dates are null', () => {
    const html = renderToString(React.createElement(OrderTimeline, { deal: makeDeal() }));
  ...
  it('marks success-toned passed event (Завершён) green and warning-toned (Дедлайн) orange', () => {
    ...
    const html = renderToString(React.createElement(OrderTimeline, { deal }));
  ...
  it('omits the footer when lastSyncedAt is null', () => {
    const html = renderToString(
      React.createElement(OrderTimeline, { deal: makeDeal({ lastSyncedAt: null }) })
    );
