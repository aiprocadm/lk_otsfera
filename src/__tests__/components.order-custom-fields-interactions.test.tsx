...
describe('OrderCustomFields — EditForm interactions (Pattern I)', () => {
  ...
  it('typing in a text input updates its value (handleChange)', () => {
    render(
      React.createElement(OrderCustomFields, { fields: [textField], orderId: 'o1', editable: true })
    );
  ...
  it('toggling a checkbox updates its checked state (handleChange, boolean branch)', () => {
    render(
      React.createElement(OrderCustomFields, { fields: [boolField], orderId: 'o1', editable: true })
    );
  ...
  it('changing a select updates its value (handleChange, select branch)', () => {
    render(
      React.createElement(OrderCustomFields, {
        fields: [selectField],
        orderId: 'o1',
        editable: true,
      })
  ...
  it('submit success: converts empty string to null, shows busy label, toasts success, and refreshes', async () => {
    ...
    await waitFor(() =>
      expect(saveCustomFieldsAction).toHaveBeenCalledWith('order', 'order-1', { 'def-text': null })
    );
    await waitFor(() =>
  ...
  it('submit with a non-empty value sends the trimmed string as-is', async () => {
    ...
    await waitFor(() =>
      expect(saveCustomFieldsAction).toHaveBeenCalledWith('order', 'order-2', {
        'def-text': 'ABC-123',
      })
    );
  });
