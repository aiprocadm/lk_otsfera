...
function render(props?: { theme?: 'light' | 'dark'; bottomBar?: React.ReactNode }) {
...
describe('AppShell (общий каркас)', () => {
  ...
  it('нижняя панель рендерится и добавляет отступ контенту', () => {
    const html = render({ bottomBar: <div>ПАНЕЛЬ</div> });
    expect(html).toContain('ПАНЕЛЬ');
    expect(html).toContain('pb-16 md:pb-0');
  });

  it('без нижней панели отступа нет', () => {
    expect(render()).not.toContain('pb-16');
  });
