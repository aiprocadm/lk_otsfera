...
const LEADER_SECTIONS = [
  ...
  // `У-222` (этап 4): правила автоматизации СВОЕЙ компании. Платформенных
  // правил нет — робот, ставящий задачи сразу во всех компаниях, это не
  // функция; администратор правит правила конкретной компании, выбрав её.
  'catalogs.automation',
...
const ADMIN_ONLY_SECTIONS = [
  ...
  'integrations.bitrix',
  // Этап 3 (`У-211`): токен формы сайта — секрет платформы (`Р-22`), и приём
  // заявок включается один раз на всю систему, а не по компаниям.
