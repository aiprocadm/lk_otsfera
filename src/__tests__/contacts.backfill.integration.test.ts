...
describe('backfillContacts', () => {
  it('seeds contacts from org-role Users and Leads; dedups by channel; is idempotent', async () => {
    ...
    await prisma.user.create({
      data: {
        email: `${STAMP}-u@t.local`,
        name: `${STAMP}-User`,
        role: 'organization',
        organizationId: org.id,
        whatsappPhone: '+79990007777',
      },
    });
    const partner = await prisma.partner.create({ data: { name: `${STAMP}-p` } });
    const creator = await prisma.user.create({
      data: { email: `${STAMP}-c@t.local`, name: 'C', role: 'partner', partnerId: partner.id },
    });
    await prisma.lead.create({
      data: {
        ...
        organizationId: org.id,
        clientCompanyName: `${STAMP}-Lead`,
        clientContactName: `${STAMP}-Контакт`,
        clientContactPhone: '+79990007777',
        subject: 's',
      },
    });
    ...
    const chans = await prisma.contactChannel.findMany({
      where: { companyId: co.id, normalizedValue: '+79990007777' },
    });
  ...
  it('user with telegram/max chat ids seeds both channel types; a blank chatId is skipped (ensureChannel !nv); a channel already claimed by another contact is not duplicated onto the new one', async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: `${STAMP}-orgU2`, companyId: co.id },
    });
    ...
    const priorContact = await prisma.contact.create({
      data: { companyId: co.id, name: `${STAMP}-PriorOwner` },
    });
    await prisma.contactChannel.create({
      data: {
        contactId: priorContact.id,
        companyId: co.id,
        type: 'email',
        value: `${STAMP}-shared@t.local`,
        normalizedValue: `${STAMP}-shared@t.local`,
      },
    });
    await prisma.user.create({
      data: {
        email: `${STAMP}-shared@t.local`,
        name: `${STAMP}-U2`,
        role: 'organization',
        organizationId: org.id,
        whatsappPhone: '+79990002020',
        telegramChatId: `${STAMP}-tg2`,
        maxChatId: '\t \t', // blank after trim → ensureChannel !nv early-return
      },
    });
    ...
    const created = await prisma.contact.findFirst({
      where: { companyId: co.id, name: { not: `${STAMP}-PriorOwner` } },
    ...
    const emailChans = await prisma.contactChannel.findMany({
      where: { companyId: co.id, normalizedValue: `${STAMP}-shared@t.local` },
    });
  ...
  it('a Lead with no organizationId is skipped (org-less leads cannot be scoped to a company)', async () => {
    ...
    const creator = await prisma.user.create({
      data: {
        email: `${STAMP}-orglessCreator@t.local`,
        name: 'C2',
        role: 'partner',
        partnerId: partner.id,
      },
    ...
    await prisma.lead.create({
      data: {
        partnerId: partner.id,
        createdByUserId: creator.id,
        clientCompanyName: `${STAMP}-OrglessLead`,
        clientContactName: `${STAMP}-OrglessContact`,
        clientContactPhone: '+79990003030',
        subject: 's',
      },
    });
  ...
  it('a fresh Lead (email + phone, neither previously seen) creates a NEW contact with both channels', async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: `${STAMP}-orgLeadFresh`, companyId: co.id },
    });
    const partner = await prisma.partner.create({ data: { name: `${STAMP}-pFresh` } });
    const creator = await prisma.user.create({
      data: {
        email: `${STAMP}-freshCreator@t.local`,
        name: 'C3',
        role: 'partner',
        partnerId: partner.id,
      },
    ...
    await prisma.lead.create({
      data: {
        ...
        createdByUserId: creator.id,
        organizationId: org.id,
        clientCompanyName: `${STAMP}-FreshLead`,
        clientContactName: `${STAMP}-FreshContact`,
        clientContactPhone: '+79990004040',
        clientContactEmail: `${STAMP}-fresh@t.local`,
        subject: 's',
      },
    });
    ...
    const contact = await prisma.contact.findFirst({
      where: { companyId: co.id, name: `${STAMP}-FreshContact` },
      include: { channels: true },
    });
  ...
  it('a Lead with a blank clientContactName falls back to the default "Контакт" name', async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: `${STAMP}-orgBlankName`, companyId: co.id },
    });
    const partner = await prisma.partner.create({ data: { name: `${STAMP}-pBlankName` } });
    const creator = await prisma.user.create({
      data: {
        email: `${STAMP}-blankNameCreator@t.local`,
        name: 'C4',
        role: 'partner',
        partnerId: partner.id,
      },
    ...
    await prisma.lead.create({
      data: {
        ...
        createdByUserId: creator.id,
        organizationId: org.id,
        clientCompanyName: `${STAMP}-BlankNameLead`,
        clientContactName: '',
        clientContactPhone: '+79990005050',
        subject: 's',
      },
    });
