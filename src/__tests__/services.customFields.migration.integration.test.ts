...
beforeAll(async () => {
  ...
  adminId = (await prisma.user.create({
    data: { email: `cfm-admin-${S}@t.local`, passwordHash: 'x', name: 'CFM Admin', role: 'admin' }
  })).id;
  managerId = (await prisma.user.create({
    data: { email: `cfm-mgr-${S}@t.local`, passwordHash: 'x', name: 'CFM Manager', role: 'manager' }
  })).id;
  ...
  orderId = (await prisma.order.create({
    data: {
      title: `CFM-Order-${S}`, orderNumber: `CFM-ON-${S}`,
      companyId, partnerId, organizationId: orgId, executionStatus: 'in_progress'
    }
  })).id;
  ...
  const legacy = await createDefinition(prisma, admin, {
    entityType: 'order', key: `cfm_legacy_${S}`, label: 'Поле до этапа 1',
    fieldType: 'text', sortOrder: 1, editableByRoles: ['admin', 'leader', 'manager']
  });
  ...
  const fresh = await createDefinition(prisma, admin, {
    entityType: 'order', key: `cfm_fresh_${S}`, label: 'Поле после этапа 1',
