...
describe('sendMessage', () => {
  it('rejects empty body', async () => {
    expect(
      await sendMessage(prismaMock(), orgSession, { orderId: 'o1', side: 'org', body: '   ' })
  ...
  it('org author succeeds and notifies managers', async () => {
    const r = await sendMessage(prismaMock(), orgSession, {
      orderId: 'o1',
      side: 'org',
      body: 'hi',
    });
  ...
  it('team (manager) on org side notifies org users', async () => {
    const r = await sendMessage(prismaMock(), teamSession, {
      orderId: 'o1',
      side: 'org',
      body: 'reply',
    });
  ...
  it('forbidden for manager of another company (C8 cross-company isolation)', async () => {
    ...
    expect(
      await sendMessage(prismaMock(), foreignManager, { orderId: 'o1', side: 'org', body: 'hi' })
    ).toEqual({ ok: false, error: 'forbidden' });
  });
  it('forbidden when partner writes org side', async () => {
    const partner = { sub: 'pu', role: 'partner', partnerId: 'p1' } as any;
    expect(
      await sendMessage(prismaMock(), partner, { orderId: 'o1', side: 'org', body: 'hi' })
    ).toEqual({ ok: false, error: 'forbidden' });
  });
  it('order_not_found', async () => {
    const pm = prismaMock();
    pm.order.findUnique.mockResolvedValue(null);
    expect(await sendMessage(pm, teamSession, { orderId: 'x', side: 'org', body: 'hi' })).toEqual({
      ok: false,
