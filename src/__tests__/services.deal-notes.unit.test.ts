...
const { getOrder, recordAudit, listColleagues, createNotification, deliverNotificationToUser } = vi.hoisted(() => ({
  getOrder: vi.fn(),
  recordAudit: vi.fn(),
...
it('mentions in note body notify mentioned staff (not the author)', async () => {
  ...
  expect(deliverNotificationToUser).toHaveBeenCalledWith(expect.objectContaining({ userId: 'u2', type: 'deal_note_mention', url: '/manager/orders/o1', dedupKey: 'n1' }));
});
...
it('admin recipient gets the notification WITHOUT url (нет /manager-кабинета)', async () => {
  ...
  const prisma = { dealNote: { create: vi.fn().mockResolvedValue({ id: 'note1' }) }, user: { findMany: vi.fn().mockResolvedValue([{ id: 'a1', role: 'admin' }]) } } as never;
...
it('self-mention is silent; notify failure does not fail the note', async () => {
  getOrder.mockResolvedValue({ id: 'o1', companyId: 'c1' });
  listColleagues.mockResolvedValue({ ok: true, rows: [{ id: 'u1', name: 'Я' }, { id: 'u2', name: 'Пётр' }] });
  createNotification.mockRejectedValueOnce(new Error('boom'));
  const prisma = { dealNote: { create: vi.fn().mockResolvedValue({ id: 'note1' }) }, user: { findMany: vi.fn().mockResolvedValue([{ id: 'u2', role: 'manager' }]) } } as never;
...
it('notify failure with a non-Error rejection value falls back to String(err)', async () => {
  ...
  const prisma = { dealNote: { create: vi.fn().mockResolvedValue({ id: 'note1' }) }, user: { findMany: vi.fn() } } as never;
