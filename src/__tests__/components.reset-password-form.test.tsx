...
describe('ResetPasswordForm', () => {
  ...
  it('error invalid_token: shows the expired-link message', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: false, json: () => Promise.resolve({ error: 'invalid_token' }) });
    vi.stubGlobal('fetch', fetchMock);
    ...
    await waitFor(() => expect(toastError).toHaveBeenCalledWith('Ссылка недействительна или истекла. Запросите новую.'));
    expect(push).not.toHaveBeenCalled();
  });

  it('error weak_password: shows the weak-password message', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: false, json: () => Promise.resolve({ error: 'weak_password' }) });
    vi.stubGlobal('fetch', fetchMock);
    render(React.createElement(ResetPasswordForm, { token: 't' }));
    const { passwordInput, confirmInput } = inputs();
    fireEvent.change(passwordInput, { target: { value: 'longenough1' } });
  ...
  it('error with an unmapped code: falls back to the generic message', async () => {
    ...
    await waitFor(() => expect(toastError).toHaveBeenCalledWith('Не удалось установить пароль. Попробуйте ещё раз.'));
  });
