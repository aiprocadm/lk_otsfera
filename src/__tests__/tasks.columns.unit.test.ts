...
describe('DEFAULT_TASK_COLUMNS', () => {
  it('4 колонки: todo/in_progress/review/done, позиции 0..3', () => {
    expect(DEFAULT_TASK_COLUMNS.map((c) => c.statusAnchor)).toEqual([
      'todo',
      'in_progress',
      'review',
      'done',
  ...
  it('единственная done-колонка — последняя (isDoneColumn)', () => {
    expect(DEFAULT_TASK_COLUMNS.filter((c) => c.isDoneColumn).map((c) => c.statusAnchor)).toEqual([
      'done',
    ]);
  });
...
describe('columnForTask', () => {
  ...
  it('явный columnId, существующий в наборе', () => {
    expect(columnForTask(columns, { status: 'todo', columnId: 'default:review' })?.id).toBe(
      'default:review'
    );
  });

  it('columnId отсутствует в наборе → колонка по якорю status', () => {
    expect(columnForTask(columns, { status: 'in_progress', columnId: 'stale-id' })?.id).toBe(
      'default:in_progress'
    );
  });
