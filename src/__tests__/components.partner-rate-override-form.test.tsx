...
describe('RateOverrideForm', () => {
  ...
  it('initializes the rate field from initialRate (as a percentage) and the reason from initialNote', () => {
    render(React.createElement(RateOverrideForm, { orgId: 'o1', initialRate: '0.085', initialNote: 'VIP' }));
  ...
  it('shows the "clear" button when initialRate is not null', () => {
    render(React.createElement(RateOverrideForm, { orgId: 'o1', initialRate: '0.1', initialNote: null }));
  ...
  it('"Сохранить" is disabled until both rate and reason are filled', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Например: VIP-клиент, индивидуальные условия'), { target: { value: 'Причина' } });
  ...
  it('"Вернуть базовую ставку" (clear): PUT rate=null + trimmed reason', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    render(React.createElement(RateOverrideForm, { orgId: 'o1', initialRate: '0.1', initialNote: 'заметка' }));
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/partner/portfolio/o1/rate',
      expect.objectContaining({ method: 'PUT', body: JSON.stringify({ rate: null, reason: 'заметка' }) })
    );
  });
  ...
  it('"Вернуть базовую ставку" with an empty reason falls back to the default audit note', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/partner/portfolio/o1/rate',
      expect.objectContaining({ body: JSON.stringify({ rate: null, reason: 'Возврат к базовой ставке' }) })
    );
  });
});
