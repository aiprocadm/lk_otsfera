...
import { isTransientSkip, capturePendingSkips, replayPendingRecords, type CursorEntity } from '@/lib/services/oneCSync/pending';
...
describe('replayPendingRecords', () => {
  ...
  it('bumps attempts and keeps pending on a repeated transient skip below the cap', async () => {
    upsertPaymentRecord.mockImplementation(async (_db: unknown, _dto: unknown, sum: { skipped: number; skips: Array<{ externalId: string; reason: string }> }) => { sum.skipped += 1; sum.skips.push({ externalId: 'P1', reason: 'organization_not_found' }); });
