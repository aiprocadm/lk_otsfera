...
function txPrisma(existingCursor: string | null) {
  const findUnique = vi
    .fn()
    .mockResolvedValue(existingCursor === undefined ? null : { cursor: existingCursor });
...
describe('rewindCursor', () => {
  it('rejects an unknown entity', async () => {
    ...
    expect(await rewindCursor(prisma, 'u1', 'nope', null)).toEqual({
      ok: false,
  ...
  it('rejects reconcile (no cursor)', async () => {
    const { prisma } = txPrisma(null);
    expect(await rewindCursor(prisma, 'u1', 'reconcile', '2026-06-01T00:00:00.000Z')).toEqual({
      ok: false,
      error: 'unknown_entity',
    });
  });
  ...
  it('rejects a future timestamp', async () => {
    ...
    expect(await rewindCursor(prisma, 'u1', 'order', future)).toEqual({
      ok: false,
  ...
  it('rejects an unparseable timestamp', async () => {
    const { prisma } = txPrisma(null);
    expect(await rewindCursor(prisma, 'u1', 'order', 'not-a-date')).toEqual({
      ok: false,
      error: 'invalid_cursor',
    });
  });
  ...
  it('returns storage on transaction failure', async () => {
    ...
    expect(await rewindCursor(prisma, 'u1', 'order', null)).toEqual({
      ok: false,
      error: 'storage',
    });
  });
...
describe('triggerSync', () => {
  it('rejects an unknown entity', async () => {
    ...
    expect(await triggerSync(prisma, 'u1', 'nope', provider)).toEqual({
      ok: false,
      error: 'unknown_entity',
    });
  });
  ...
  it('refuses when a run is already active', async () => {
    ...
    expect(await triggerSync(prisma, 'u1', 'order', provider)).toEqual({
      ok: false,
  ...
  it('enqueues a manual job and audits on success', async () => {
    ...
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ jobId: expect.stringMatching(/^manual:order:\d+$/) })
    );
    expect(create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ entity: 'sync_state', action: 'sync_triggered' }),
      })
    );
  });
...
describe('triggerSync — background cron jobs (G3)', () => {
  it.each([
    ...
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        jobId: expect.stringMatching(new RegExp(`^manual:${entity}:\\d+$`)),
      })
    );
    expect(create).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('already_running guard applies to background jobs too', async () => {
    ...
    expect(await triggerSync(prisma, 'u1', 'emailPoll', provider)).toEqual({
      ok: false,
      error: 'already_running',
    });
  ...
  it.each([['certificateExpiry'], ['emailPoll'], ['mangoBackfill'], ['monthlyCommissions']])(
    ...
    async (entity) => {
      const { prisma } = txPrisma(null);
      expect(await rewindCursor(prisma, 'u1', entity, null)).toEqual({
        ok: false,
        error: 'unknown_entity',
      });
    }
...
describe('setSchedulePaused', () => {
  it('rejects an unknown schedulerId', async () => {
    ...
    expect(await setSchedulePaused(prisma, 'u1', 'bogus.cron', true, provider)).toEqual({
      ok: false,
  ...
  it('pause: writes DB row then removes the live scheduler', async () => {
    ...
    expect(upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { schedulerId: 'oneCSync.pullOrders.cron' } })
    );
  ...
  it('resume: deletes DB row then re-registers the scheduler', async () => {
    ...
    expect(upsertJobScheduler).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ tz: 'Europe/Moscow' }),
      expect.anything()
    );
  });
  ...
  it('returns queue_unavailable when the scheduler op throws (DB intent kept)', async () => {
    ...
    const provider: SyncControlQueueProvider = () =>
      ({
        ...
        add: vi.fn(),
        upsertJobScheduler: vi.fn(),
        removeJobScheduler: vi.fn().mockRejectedValue(new Error('redis down')),
      }) as never;
    expect(
      await setSchedulePaused(prisma, 'u1', 'oneCSync.pullOrders.cron', true, provider)
    ).toEqual({ ok: false, error: 'queue_unavailable' });
  });
  ...
  it.each([['notifications.certificateExpiry.cron'], ['docs.calculateMonthlyCommissions.cron']])(
    ...
    async (schedulerId) => {
      ...
      expect(await setSchedulePaused(prisma, 'u1', schedulerId, true, provider)).toEqual({
        ok: false,
        error: 'unknown_schedule',
      });
  ...
  it.each([['certificateExpiry'], ['emailPoll'], ['mangoBackfill'], ['monthlyCommissions']])(
    ...
    async (entityKey) => {
      ...
      expect(await setSchedulePaused(prisma, 'u1', entityKey, true, provider)).toEqual({
        ok: false,
        error: 'unknown_schedule',
      });
      expect(upsert).not.toHaveBeenCalled();
