  ...
  it('returns org_not_found when org does not exist', async () => {
    ...
    expect(
      await createAndAssignManager(
        p,
        { mode: 'existing', organizationId: 'nonexistent', email: 'x@t.local' },
        'admin-1'
      )
...
describe('createAndAssignManager — mode=existing', () => {
  it('returns user_not_found when email is unknown', async () => {
    ...
    expect(
      await createAndAssignManager(
        p,
        { mode: 'existing', organizationId: 'org-1', email: 'ghost@t.local' },
        'admin-1'
      )
  ...
  it('returns role_conflict when user exists but is not a manager', async () => {
    ...
    expect(
      await createAndAssignManager(
        p,
        { mode: 'existing', organizationId: 'org-1', email: existingNonMgr.email },
        'admin-1'
      )
  ...
  it('succeeds for existing manager with password (no invite URL, alreadyHasPassword=true)', async () => {
    ...
    tx.organizationManager.create = vi
      .fn()
      .mockResolvedValue({ id: 'assign-1', organizationId: 'org-1' });
    const p = makePrisma(tx);
    const result = await createAndAssignManager(
      p,
      { mode: 'existing', organizationId: 'org-1', email: existingMgr.email },
      'admin-1'
    );
...
describe('createAndAssignManager — APP_URL env branch', () => {
  it('uses APP_URL env var for invite URL when set', async () => {
    ...
    try {
      ...
      tx.organizationManager.create = vi
        .fn()
        .mockResolvedValue({ id: 'assign-env', organizationId: 'org-1' });
      const p = makePrisma(tx);
      const result = await createAndAssignManager(
        p,
        { mode: 'new', organizationId: 'org-1', email: newUser.email, name: 'Env test' },
        'admin-1'
      );
      if (!result.ok) throw new Error('expected ok');
...
describe('createAndAssignManager — mode=new', () => {
  it('creates new user and mints invite URL (passwordHash=null)', async () => {
    ...
    tx.organizationManager.create = vi
      .fn()
      .mockResolvedValue({ id: 'assign-2', organizationId: 'org-1' });
    const p = makePrisma(tx);
    const result = await createAndAssignManager(
      p,
  ...
  it('uses email as name when name is not provided', async () => {
    ...
    tx.organizationManager.create = vi
      .fn()
      .mockResolvedValue({ id: 'assign-3', organizationId: 'org-1' });
    const p = makePrisma(tx);
    await createAndAssignManager(
      p,
      { mode: 'new', organizationId: 'org-1', email: 'user@t.local' },
      'admin-1'
    );
    expect(tx.user.create).toHaveBeenCalledWith({
  ...
  it('returns role_conflict for mode=new when existing user is not a manager', async () => {
    ...
    expect(
      await createAndAssignManager(
        p,
        { mode: 'new', organizationId: 'org-1', email: existingNonMgr.email },
        'admin-1'
      )
    ).toEqual({ ok: false, error: 'role_conflict' });
  });
  ...
  it('reuses existing manager account for mode=new (passwordHash set)', async () => {
    ...
    tx.organizationManager.create = vi
      .fn()
      .mockResolvedValue({ id: 'assign-4', organizationId: 'org-1' });
    const p = makePrisma(tx);
    const result = await createAndAssignManager(
      p,
      { mode: 'new', organizationId: 'org-1', email: existingMgr.email },
      'admin-1'
    );
    if (!result.ok) throw new Error('expected ok');
...
describe('createAndAssignManager — assignment reactivation', () => {
  it('reactivates inactive assignment instead of creating new one', async () => {
    ...
    tx.organizationManager.update = vi
      .fn()
      .mockResolvedValue({ id: 'assign-old', organizationId: 'org-1' });
    const p = makePrisma(tx);
    const result = await createAndAssignManager(
      p,
      { mode: 'existing', organizationId: 'org-1', email: existingMgr.email },
...
describe('createAndAssignManager — C8 company floor', () => {
  it('rejects an existing manager whose companyId differs from the org (company_mismatch)', async () => {
    ...
    expect(
      await createAndAssignManager(
        p,
        { mode: 'existing', organizationId: 'org-1', email: foreignMgr.email },
        'admin-1'
      )
  ...
  it('allows an existing manager in the same company as the org', async () => {
    ...
    const result = await createAndAssignManager(
      p,
      { mode: 'existing', organizationId: 'org-1', email: sameCompanyMgr.email },
      'admin-1'
    );
    if (!result.ok) throw new Error('expected ok');
  ...
  it('stamps a newly-created manager with the org companyId (mode=new)', async () => {
    ...
    const result = await createAndAssignManager(
      p,
      { mode: 'new', organizationId: 'org-1', email: created.email, name: 'Fresh' },
      'admin-1'
    );
    if (!result.ok) throw new Error('expected ok');
    expect(tx.user.create).toHaveBeenCalledWith({
...
describe('deactivateAssignment', () => {
  ...
  it('ФТ-11.2: инкремент идёт внутри той же транзакции, что и organizationManager.update', async () => {
    ...
    expect(
      (p as unknown as { user: { update: ReturnType<typeof vi.fn> } }).user.update
