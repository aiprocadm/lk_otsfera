...
const chan = (over: any = {}) => ({
  contact: { id: 'k1', organizationId: 'o1', companyId: 'c1', userId: 'u1', isArchived: false },
  ...over,
});
...
describe('resolveContactByChannel', () => {
  ...
  it('ambiguous (>1, e.g. cross-company) → null (never guess)', async () => {
    const r = await resolveContactByChannel(
      db([chan(), chan({ contact: { id: 'k2', organizationId: 'o2', companyId: 'c2' } })]),
  ...
  it('archived contact is ignored', async () => {
    const r = await resolveContactByChannel(
      db([
        chan({ contact: { id: 'k1', organizationId: 'o1', companyId: 'c1', isArchived: true } }),
      ]),
      { type: 'phone', value: '+7999' }
    );
    expect(r).toBeNull();
  });
