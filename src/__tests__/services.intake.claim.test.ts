...
describe('claimEnrollment', () => {
  ...
  it('клиентская роль → forbidden (без чтения БД)', async () => {
    ...
    expect(await claimEnrollment(prisma, partner(), { id: 'e1' })).toEqual({
      ok: false,
      error: 'forbidden',
    });
  ...
  it('успех: пишет claimed-поля + аудит', async () => {
    ...
    expect(await claimEnrollment(prisma, manager(), { id: 'e1' })).toEqual({
      ok: true,
      changed: true,
    });
    ...
    expect(recordAuditMock).toHaveBeenCalledWith(
      prisma,
      expect.objectContaining({ action: 'intake_claimed', entity: 'enrollment_request' })
    );
  });

  it('not_found / не-pending / чужой claim / повторный свой', async () => {
    expect(
      await claimEnrollment(fake('enrollmentRequest', null).prisma, manager(), { id: 'x' })
    ).toEqual({ ok: false, error: 'not_found' });
    expect(
      await claimEnrollment(
        fake('enrollmentRequest', { ...ROW, status: 'approved' }).prisma,
        manager(),
        { id: 'e1' }
      )
    ...
    expect(
      await claimEnrollment(
        fake('enrollmentRequest', { ...ROW, claimedByUserId: 'm2' }).prisma,
        manager(),
        { id: 'e1' }
      )
    ).toEqual({ ok: false, error: 'already_assigned' });
    const own = fake('enrollmentRequest', { ...ROW, claimedByUserId: 'm1' });
    expect(await claimEnrollment(own.prisma, manager(), { id: 'e1' })).toEqual({
      ok: true,
      changed: false,
    });
...
describe('claimInbound', () => {
  ...
  it('успех по общей очереди (companyId=null) + аудит', async () => {
    ...
    expect(await claimInbound(prisma, manager(), { id: 'i1' })).toEqual({
      ok: true,
      changed: true,
    });
  ...
  it('чужая компания при не-unresolved → not_found (scope)', async () => {
    ...
    expect(await claimInbound(fake('inboundMessage', row).prisma, manager(), { id: 'i1' })).toEqual(
  ...
  it('своя компания, но статус bound → lifecycle_violation', async () => {
    ...
    expect(await claimInbound(fake('inboundMessage', row).prisma, manager(), { id: 'i1' })).toEqual(
      { ok: false, error: 'lifecycle_violation' }
    );
  });
  ...
  it('чужой claim → already_assigned; свой → идемпотентно', async () => {
    expect(
      await claimInbound(
        fake('inboundMessage', { ...ROW, claimedByUserId: 'm2' }).prisma,
        manager(),
        { id: 'i1' }
      )
    ).toEqual({ ok: false, error: 'already_assigned' });
    expect(
      await claimInbound(
        fake('inboundMessage', { ...ROW, claimedByUserId: 'm1' }).prisma,
        manager(),
        { id: 'i1' }
      )
  ...
  it('forbidden для клиентской роли', async () => {
    expect(await claimInbound(fake('inboundMessage', ROW).prisma, partner(), { id: 'i1' })).toEqual(
...
describe('claimCall / closeCallIntake', () => {
  ...
  it('claim: успех, чужая компания → not_found, закрытый → lifecycle', async () => {
    expect(await claimCall(fake('call', ROW).prisma, manager(), { id: 'c1' })).toEqual({
      ok: true,
      changed: true,
    });
    expect(
      await claimCall(fake('call', { ...ROW, companyId: 'co-B' }).prisma, manager(), { id: 'c1' })
    ).toEqual({ ok: false, error: 'not_found' });
    expect(
      await claimCall(fake('call', { ...ROW, intakeClosedAt: new Date() }).prisma, manager(), {
        id: 'c1',
      })
    ).toEqual({ ok: false, error: 'lifecycle_violation' });
    expect(
      await claimCall(fake('call', { ...ROW, claimedByUserId: 'm2' }).prisma, manager(), {
        id: 'c1',
      })
    ).toEqual({ ok: false, error: 'already_assigned' });
    expect(
      await claimCall(fake('call', { ...ROW, claimedByUserId: 'm1' }).prisma, manager(), {
        id: 'c1',
      })
    ).toEqual({ ok: true, changed: false });
  });
  ...
  it('claim: клиентская роль → forbidden, БД не читаем', async () => {
    ...
    expect(await claimCall(f.prisma, partner(), { id: 'c1' })).toEqual({
      ok: false,
      error: 'forbidden',
    });
  ...
  it('claim: сессия без companyId видит только общую корзину', async () => {
    ...
    expect(
      await claimCall(fake('call', row).prisma, manager({ companyId: null }), { id: 'c1' })
    ).toEqual({ ok: false, error: 'not_found' });
  });

  it('close: успех (+аудит), повторно — идемпотентно, forbidden для клиента', async () => {
    const first = fake('call', ROW);
    expect(await closeCallIntake(first.prisma, manager(), { id: 'c1' })).toEqual({
      ok: true,
      changed: true,
    });
    ...
    expect(recordAuditMock).toHaveBeenCalledWith(
      first.prisma,
      expect.objectContaining({ action: 'intake_call_closed' })
    );

    expect(
      await closeCallIntake(
        fake('call', { ...ROW, intakeClosedAt: new Date() }).prisma,
        manager(),
        { id: 'c1' }
      )
    ).toEqual({ ok: true, changed: false });
    expect(await closeCallIntake(fake('call', ROW).prisma, partner(), { id: 'c1' })).toEqual({
      ok: false,
      error: 'forbidden',
    });
    expect(await closeCallIntake(fake('call', null).prisma, manager(), { id: 'c1' })).toEqual({
      ok: false,
      error: 'not_found',
    });
  });
