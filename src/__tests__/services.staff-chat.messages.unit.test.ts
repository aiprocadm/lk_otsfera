...
function prismaFixture(over: Record<string, unknown> = {}) {
  const base = {
    ...
    user: {
      findMany: vi.fn().mockResolvedValue([]),
      findUnique: vi.fn().mockResolvedValue({ id: 'm2', role: 'manager' }),
    },
  };
...
it('rejects non-staff / empty / too long', async () => {
  expect(
    await sendStaffMessage({} as never, partner, { conversationId: 'conv1', body: 'x' })
  ).toEqual({ ok: false, error: 'forbidden' });
  expect(
    await sendStaffMessage(prismaFixture(), manager, { conversationId: 'conv1', body: '   ' })
  ).toEqual({ ok: false, error: 'empty_body' });
  expect(
    await sendStaffMessage(prismaFixture(), manager, {
...
it('DM: first-unread notification once (второе сообщение подряд молчит)', async () => {
  ...
  expect(deliverNotificationToUser).toHaveBeenCalledWith(
    expect.objectContaining({ userId: 'm2', type: 'staff_dm_message', dedupKey: 'n1' })
  );
  vi.clearAllMocks();
...
it('mention notifies mentioned staff (не автора), general без per-message уведомлений', async () => {
  ...
  expect(deliverNotificationToUser).toHaveBeenCalledWith(
    expect.objectContaining({ userId: 'm2', type: 'staff_chat_mention' })
  );
});
...
it('attachment: чужой префикс → forbidden; свой → scanStatus pending + enqueue kind staff_attachment', async () => {
  ...
  expect(
    await sendStaffMessage(prisma, manager, {
      conversationId: 'conv1',
      body: 'f',
      attachmentPath: 'staff-chat/OTHER/file',
    })
  ).toEqual({ ok: false, error: 'forbidden' });
  await sendStaffMessage(prisma, manager, {
    conversationId: 'conv1',
    body: 'файл',
    attachmentPath: 'staff-chat/conv1/abc-file.pdf',
    attachmentName: 'file.pdf',
    attachmentMime: 'application/pdf',
  });
  const createArg = (
    prisma as never as { staffMessage: { create: ReturnType<typeof vi.fn> } }
  ).staffMessage.create.mock.calls.at(-1)![0];
...
it('toggleReaction: вне набора → invalid; добавление и снятие', async () => {
  ...
  expect(await toggleReaction(prisma, manager, { messageId: 'msg1', emoji: '🤡' })).toEqual({
    ok: false,
    error: 'invalid',
  });
...
it('listStaffMessages: forbidden для не-участника dm; after-курсор игнорирует NaN', async () => {
  const prisma = prismaFixture({
    staffConversation: {
      findUnique: vi
        .fn()
        .mockResolvedValue(convFixture({ participants: [{ userId: 'x' }, { userId: 'y' }] })),
      update: vi.fn(),
    },
  });
  expect(await listStaffMessages(prisma, manager, { conversationId: 'conv1' })).toEqual({
    ok: false,
    error: 'forbidden',
  });
});
...
it('sendStaffMessage: не-участник dm не может писать → forbidden', async () => {
  ...
  expect(await sendStaffMessage(prisma, manager, { conversationId: 'conv1', body: 'hi' })).toEqual({
    ok: false,
    error: 'forbidden',
  });
});
...
it('sendStaffMessage: conversation_not_found', async () => {
  ...
  expect(
    await sendStaffMessage(prisma, manager, { conversationId: 'missing', body: 'hi' })
  ).toEqual({ ok: false, error: 'conversation_not_found' });
});
...
it('sendStaffMessage: ЛС-уведомление админу использует /admin/messages', async () => {
  const prisma = prismaFixture({
    user: {
      findMany: vi.fn().mockResolvedValue([]),
      findUnique: vi.fn().mockResolvedValue({ id: 'm2', role: 'admin' }),
    },
  });
...
it('sendStaffMessage: поднимает read-state автора (своя отправка не зажигает автору его же unread)', async () => {
  const prisma = prismaFixture();
  await sendStaffMessage(prisma, manager, { conversationId: 'conv1', body: 'привет' });
  const upsert = (prisma as never as { staffMessageRead: { upsert: ReturnType<typeof vi.fn> } })
...
it('listStaffMessages: не-staff вызывающий → forbidden без обращения к prisma', async () => {
  expect(await listStaffMessages({} as never, partner, { conversationId: 'conv1' })).toEqual({
    ok: false,
    error: 'forbidden',
  });
});
...
it('listStaffMessages: conversation_not_found', async () => {
  ...
  expect(await listStaffMessages(prisma, manager, { conversationId: 'missing' })).toEqual({
    ok: false,
...
it('listStaffMessages: happy path — агрегация реакций (incl. mine) + валидный after-курсор', async () => {
  ...
  const res = await listStaffMessages(prisma, manager, {
    conversationId: 'conv1',
    after: '2026-07-17T09:00:00Z',
  });
  expect(res).toEqual({
    ok: true,
    rows: [
  ...
  const findManyCall = (prisma as never as { staffMessage: { findMany: ReturnType<typeof vi.fn> } })
    .staffMessage.findMany.mock.calls[0][0];
...
it('toggleReaction: не-staff вызывающий → forbidden', async () => {
  expect(await toggleReaction({} as never, partner, { messageId: 'msg1', emoji: '👍' })).toEqual({
    ok: false,
    error: 'forbidden',
  });
});
...
it('toggleReaction: message_not_found', async () => {
  ...
  expect(await toggleReaction(prisma, manager, { messageId: 'missing', emoji: '👍' })).toEqual({
    ok: false,
...
it('toggleReaction: повторный тоггл снимает существующую реакцию', async () => {
  ...
  expect(
    (prisma as never as { staffReaction: { delete: ReturnType<typeof vi.fn> } }).staffReaction
      .delete
...
it('toggleReaction: не-P2002 ошибка create пробрасывается', async () => {
  ...
  await expect(toggleReaction(prisma, manager, { messageId: 'msg1', emoji: '👍' })).rejects.toThrow(
    'db down'
  );
});
