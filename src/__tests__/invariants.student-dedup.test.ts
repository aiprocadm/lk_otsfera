 ...
 *   4. ДО ЭТАПА 5 (08.2026) Student намеренно НЕ хранил СНИЛС/дату рождения.
 *      **Это решение развёрнуто действующим ТЗ** (`У-20`/`У-22`): справочник
 *      сотрудников хранит их и дедуплицирует по СНИЛС → ФИО+ДР → ФИО+email.
 *      Проверки ниже переписаны под новое требование — см. CLAUDE.md §14,
 *      ловушка 4: стража прежнего решения не «чинят» откатом кода.
 *
...
describe('Справочник сотрудников (Student): email — БД-инвариант, СНИЛС и ДР хранятся (этап 5)', () => {
  ...
  /**
   * **Решение развёрнуто действующим ТЗ (этап 5, `У-20`/`У-22`).**
   *
   * Раньше здесь стояла обратная проверка: «`Student` намеренно НЕ хранит
   * СНИЛС и дату рождения, дедупликация по ним невозможна по построению».
   * Это было верно для программы v0.5, но ТЗ понятности от 08.08.2026 прямо
   * требует хранить их в справочнике и дедуплицировать по СНИЛС.
   *
   * Такие стражи по правилу CLAUDE.md §14 (ловушка 4) **переписываются под
   * новое требование, а не «чинятся» откатом кода**.
   */
  it('У-20: Student хранит СНИЛС и дату рождения — они нужны справочнику', () => {
    const studentFields = (student?.fields ?? []).map((f) => f.name);
    expect(studentFields).toEqual(
      expect.arrayContaining(['snils', 'birthDate', 'phone', 'note', 'position'])
    );
    // У позиции заявки эти поля тоже остаются: там они про конкретное обучение.
    const itemFields = (item?.fields ?? []).map((f) => f.name);
    expect(itemFields).toEqual(expect.arrayContaining(['snils', 'birthDate', 'fullName']));
  });

  it('У-21: email у сотрудника необязателен — рабочих обучают без корпоративной почты', () => {
    const email = (student?.fields ?? []).find((f) => f.name === 'email');
