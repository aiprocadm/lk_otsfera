...
describe('У-94: подстановка из ЕГРЮЛ на живом Postgres', () => {
  ...
  it('свободный ИНН записывается вместе с остальными отмеченными полями', async () => {
    /**
     * «Свободный» ИНН обязан ОТЛИЧАТЬСЯ от занятого, и это должно быть видно
     * из формулы, а не выходить случайно.
     *
     * Прежняя формула `INN.slice(0, 8) + '77'` совпадала с `INN` каждый раз,
     * когда метка времени оканчивалась на «77», — то есть примерно один
     * прогон из ста падал с «свободный ИНН занят». Плавающий тест хуже
     * отсутствующего: он приучает считать красный прогон случайностью.
     */
    const freeInn = `78${INN.slice(2)}`;
    expect(freeInn).not.toBe(INN);
