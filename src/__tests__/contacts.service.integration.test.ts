...
describe('contacts service', () => {
  it('createContact: org-less contact scoped to company, normalizes channel values', async () => {
    ...
    const r = await createContact(prisma, session(co.id), {
      name: `${STAMP}-Иван`,
      channels: [{ type: 'phone', value: '8 (999) 000-11-22' }],
    });
  ...
  it('createContact rejects a session without companyId', async () => {
    const r = await createContact(prisma, session(null as any), {
      name: `${STAMP}-x`,
      channels: [],
    });
  ...
  it('createContact (C8): refuses an organizationId belonging to ANOTHER company — no row persisted', async () => {
    ...
    const orgB = await prisma.organization.create({
      data: { name: `${STAMP}-scOrgB`, companyId: coB.id },
    });
  ...
  it('createContact: refuses an in-company org NOT in managedOrgIds when teamMode is OFF', async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: `${STAMP}-scGateOrg`, companyId: co.id },
    });
    const r = await createContact(prisma, session(co.id, []), {
      name: `${STAMP}-scGate`,
  ...
  it('createContact: allows an in-company org that IS in managedOrgIds', async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: `${STAMP}-scOkOrg`, companyId: co.id },
    });
  ...
  it('captureChannel: a value that normalizes to empty is a no-op (no query, no row)', async () => {
    ...
    const c = await prisma.contact.create({
      data: { companyId: co.id, name: `${STAMP}-EmptyChanContact` },
    });
    await captureChannel(prisma, {
      contactId: c.id,
      companyId: co.id,
  ...
  it('captureChannel: a P2002 race on create (concurrent writer already inserted the row) is swallowed, not thrown', async () => {
    ...
    const c = await prisma.contact.create({
      data: { companyId: co.id, name: `${STAMP}-RaceContact` },
    });
  ...
  it('captureChannel: a non-P2002 error on create propagates unchanged', async () => {
    ...
    const c = await prisma.contact.create({
      data: { companyId: co.id, name: `${STAMP}-RaceOtherContact` },
    });
