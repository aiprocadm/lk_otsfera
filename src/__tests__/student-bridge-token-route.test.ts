...
describe('POST /api/student/bridge/token', () => {
  ...
  it('returns safe error for missing and invalid code', async () => {
    ...
    const missingRes = await POST(buildReq({}, headers));
    expect(missingRes.status).toBe(400);
    expect(await missingRes.json()).toEqual({ error: 'invalid exchange request' });

    findUnique.mockResolvedValueOnce(null);
    const invalidRes = await POST(buildReq({ code: 'bad-code' }, headers));
  ...
  it('handles expired grant', async () => {
    ...
    const res = await POST(buildReq({ code: 'abc123' }, { 'x-bridge-client': 'svc-a', 'x-bridge-secret': 'shared-secret' }));
