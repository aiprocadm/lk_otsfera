...
beforeAll(async () => {
  ...
  plainMgrAId = (
    await prisma.user.create({
      data: { email: `fsMgr-${STAMP}@t.local`, name: 'M', role: 'manager', companyId: companyA },
    })
  ).id;
  userBId = (
    await prisma.user.create({
      data: { email: `fsUserB-${STAMP}@t.local`, name: 'B', role: 'admin', companyId: companyB },
    })
  ).id;
  partnerId = (
    await prisma.partner.create({ data: { name: `fsP-${STAMP}`, slug: `fsP-${STAMP}` } })
  ).id;
});

afterAll(async () => {
  await prisma.auditLog.deleteMany({
    where: { userId: { in: [leaderAId, plainMgrAId, userBId] } },
  });
...
describe('createFunnelStage', () => {
  it('leader creates; appears in list', async () => {
    const res = await createFunnelStage(
      prisma,
      leaderA(),
      input({ name: `Предложение-${STAMP}`, position: 10 })
    );
  ...
  it('duplicate position in company → position_taken', async () => {
    ...
    expect(await createFunnelStage(prisma, leaderA(), input({ position: 20 }))).toEqual({
      ok: false,
      error: 'position_taken',
    });
  });
  it('empty name → validation', async () => {
    expect(await createFunnelStage(prisma, leaderA(), input({ name: '  ', position: 30 }))).toEqual(
      { ok: false, error: 'validation' }
    );
  });
  it('plain manager → forbidden', async () => {
    expect(await createFunnelStage(prisma, plainMgrA(), input({ position: 40 }))).toEqual({
      ok: false,
      error: 'forbidden',
    });
  });
  it('color: не-hex строка → validation; валидный #RRGGBB → ok и сохраняется', async () => {
    expect(
      await createFunnelStage(prisma, leaderA(), input({ position: 70, color: '#ZZZZZZ' }))
    ).toEqual({ ok: false, error: 'validation' });
    const ok = await createFunnelStage(
      prisma,
      leaderA(),
      input({ position: 70, name: `Цвет-${STAMP}`, color: '#22C55E' })
    );
  ...
  it('color: undefined → ok (nullish), сохраняется null', async () => {
    const ok = await createFunnelStage(
      prisma,
      leaderA(),
      input({ position: 71, name: `БезЦвета-${STAMP}`, color: undefined })
    );
    expect(ok.ok).toBe(true);
    const id = ok.ok ? ok.id : '';
...
describe('updateFunnelStage', () => {
  it('updates + audit; cross-company → not_found', async () => {
    const created = await createFunnelStage(
      prisma,
      leaderA(),
      input({ position: 50, name: `Upd-${STAMP}` })
    );
    ...
    expect(
      await updateFunnelStage(
        ...
        leaderA(),
        id,
        input({ position: 50, name: `Upd2-${STAMP}`, isTerminal: true })
      )
    ).toEqual({ ok: true });
    ...
    expect(
      await prisma.auditLog.findFirst({
        where: { entity: 'funnel_stage', entityId: id, action: 'funnel_stage_updated' },
      })
    ).not.toBeNull();

    const inB = await createFunnelStage(
      prisma,
      adminB(),
      input({ position: 1, name: `B-${STAMP}` })
    );
    const idB = inB.ok ? inB.id : '';
    expect(await updateFunnelStage(prisma, leaderA(), idB, input({ position: 1 }))).toEqual({
      ok: false,
      error: 'not_found',
    });
  });
...
describe('deleteFunnelStage', () => {
  it('deletes; assigned lead funnelStageId nulled (FK SET NULL)', async () => {
    const created = await createFunnelStage(
      prisma,
      leaderA(),
      input({ position: 60, name: `Del-${STAMP}` })
    );
    const id = created.ok ? created.id : '';
    const leadId = (
    ...
    expect(
      (await prisma.lead.findUniqueOrThrow({ where: { id: leadId } })).funnelStageId
    ).toBeNull();
  });
  it('cross-company delete → not_found', async () => {
    const inB = await createFunnelStage(
      prisma,
      adminB(),
      input({ position: 2, name: `DelB-${STAMP}` })
    );
    const idB = inB.ok ? inB.id : '';
    expect(await deleteFunnelStage(prisma, leaderA(), idB)).toEqual({
      ok: false,
      error: 'not_found',
    });
  });
