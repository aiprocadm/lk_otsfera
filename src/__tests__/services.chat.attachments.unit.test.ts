...
function makePrisma(
  order: object | null = { id: 'o1', organizationId: 'org1', partnerId: 'p1', companyId: 'c1' }
) {
  return {
...
describe('uploadChatAttachment — unit', () => {
  ...
  it('returns order_not_found when order does not exist', async () => {
    ...
    const result = await uploadChatAttachment(makePrisma(null), session, {
      orderId: 'x',
      side: 'org',
      file: validFile,
    });
...
describe('getChatAttachmentSignedUrl — unit', () => {
  ...
  it('returns not_found when attachmentPath does not start with chat/ (belt-and-suspenders)', async () => {
    const msg = {
      id: 'm1',
      attachmentPath: 'documents/some/old-path.pdf',
      thread: {
        side: 'org',
        order: { id: 'o1', organizationId: 'org1', partnerId: 'p1', companyId: 'c1' },
      },
