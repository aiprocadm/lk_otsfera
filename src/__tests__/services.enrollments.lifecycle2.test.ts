...
const { notifySubmitterEnrollmentStatus } = vi.hoisted(() => ({ notifySubmitterEnrollmentStatus: vi.fn() }));
...
describe('ENROLLMENT_PIPELINE (порядок конвейера)', () => {
  it('pending → approved → provisioned → in_training → certificates_ready', () => {
    expect(ENROLLMENT_PIPELINE).toEqual(['pending', 'approved', 'provisioned', 'in_training', 'certificates_ready']);
  });
});
...
describe('aggregateEnrollmentHeaderStatus (минимальный по конвейеру среди не-отклонённых)', () => {
  ...
  it('provisioned+in_training → provisioned', () => {
    expect(aggregateEnrollmentHeaderStatus([st('provisioned'), st('in_training')])).toBe('provisioned');
  });
  it('все certificates_ready → certificates_ready', () => {
    expect(aggregateEnrollmentHeaderStatus([st('certificates_ready'), st('certificates_ready')])).toBe('certificates_ready');
  });
  it('rejected игнорируются: rejected+in_training → in_training', () => {
    expect(aggregateEnrollmentHeaderStatus([st('rejected'), st('in_training')])).toBe('in_training');
  });
...
describe('advanceEnrollmentItems (bulk provisioned → in_training → certificates_ready)', () => {
  ...
  it('not_found на отсутствующей заявке', async () => {
    ...
    (d as { enrollmentRequest: { findUnique: ReturnType<typeof vi.fn> } }).enrollmentRequest.findUnique = vi
      .fn()
  ...
  it('itemIds: [] → validation (пустой явный выбор)', async () => {
    ...
    expect(await advanceEnrollmentItems(d, { ...ARGS, itemIds: [] })).toEqual({ ok: false, error: 'validation' });
  });

  it('неизвестный itemId → validation', async () => {
    const { d } = db('provisioned', [{ id: 'i1', status: 'provisioned' }]);
    expect(await advanceEnrollmentItems(d, { ...ARGS, itemIds: ['iX'] })).toEqual({ ok: false, error: 'validation' });
  });
  ...
  it('без itemIds и нет позиций на предыдущем шаге → lifecycle_violation', async () => {
    const { d } = db('approved', [{ id: 'i1', status: 'approved' }, { id: 'i2', status: 'rejected' }]);
    expect(await advanceEnrollmentItems(d, ARGS)).toEqual({ ok: false, error: 'lifecycle_violation' });
  });
  ...
  it('успех: все provisioned → in_training, шапка in_training, податель уведомлён, аудит без ПДн', async () => {
    ...
    expect(requestUpdate).toHaveBeenCalledWith({ where: { id: 'E1' }, data: { status: 'in_training' } });
    expect(notifySubmitterEnrollmentStatus).toHaveBeenCalledTimes(1);
    expect(notifySubmitterEnrollmentStatus.mock.calls[0][1]).toMatchObject({ id: 'E1', status: 'in_training' });
  ...
  it('частичный переход (1 из 2): шапка остаётся provisioned (минимум), уведомления нет', async () => {
    ...
    expect(requestUpdate).toHaveBeenCalledWith({ where: { id: 'E1' }, data: { status: 'provisioned' } });
  ...
  it('certificates_ready: предыдущий шаг in_training, rejected не мешает шапке', async () => {
    ...
    const r = await advanceEnrollmentItems(d, { id: 'E1', reviewerId: 'm1', target: 'certificates_ready' });
    if (!r.ok) throw new Error('expected ok');
    expect(r.headerChanged).toBe(true);
...
describe('approve/reject/markProvisioned уведомляют подателя (best-effort в notify.ts)', () => {
  ...
  it('approve: уведомление с обновлённой шапкой approved', async () => {
    ...
    if (!r.ok) throw new Error('expected ok');
    expect(notifySubmitterEnrollmentStatus).toHaveBeenCalledTimes(1);
    expect(notifySubmitterEnrollmentStatus.mock.calls[0][1]).toMatchObject({ id: 'E1', status: 'approved' });
  });

  it('reject: уведомление с rejected и причиной', async () => {
    const r = await rejectEnrollment(db('pending').d, { id: 'E1', reviewerId: 'm1', reason: 'нет мест' });
    if (!r.ok) throw new Error('expected ok');
  ...
  it('markProvisioned: уведомление с provisioned', async () => {
    const r = await markProvisioned(db('approved').d, { id: 'E1', reviewerId: 'm1', externalStudentId: 'LMS-9' });
    if (!r.ok) throw new Error('expected ok');
    expect(notifySubmitterEnrollmentStatus.mock.calls[0][1]).toMatchObject({ status: 'provisioned' });
  });
