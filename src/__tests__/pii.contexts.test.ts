...
describe('PII_CONTEXTS registry', () => {
  ...
  it('содержит все 34 контекста v1+M1+M6+этапы 2/5/7/9 + ТЗ кабинетов + мессенджеры + контакты + миграция + диалоги в карточках', () => {
    expect(entries.map(([k]) => k).sort()).toEqual([
      ...
      // `У-210`: переписка видна прямо в карточке заказа и организации. Это
      // те же ПДн собеседников, что и в разделе «Мессенджеры», просто прочтённые
      // из другого места — значит, и в журнале доступа они отмечаются отдельно,
      // иначе по журналу нельзя понять, откуда человек их смотрел.
      'order_card_dialogs',
      'order_items_list',
      'org_card_calls',
      'org_card_certificates_export', // этап 9 PR-3: выгрузка удостоверений из карточки
      'org_card_dialogs', // `У-210`: вкладка «Диалоги» карточки организации
