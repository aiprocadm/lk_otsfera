...
const NOT_A_LIST_SITES: Record<string, string> = {
  ...
  'src/lib/services/bitrix/mapping/lookup.ts::loadDocuments':
    'Проверка «этот файл Битрикса уже переносили» по `bitrixId` — не витрина, а защита от дубля. Заменённая версия здесь как раз нужна: файл перенесли, потом заменили новой версией — переносить его второй раз всё равно не надо.',
};
