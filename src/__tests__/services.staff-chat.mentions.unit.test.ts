...
it('listColleagues: admin — без фильтра по компании (companyId: undefined)', async () => {
  ...
  expect(findMany).toHaveBeenCalledWith(
    expect.objectContaining({
      where: {
        isActive: true,
        OR: [{ role: { in: ['manager', 'leader'] }, companyId: undefined }, { role: 'admin' }],
      },
    })
  );
});
...
it('listColleagues: manager со своей компанией — скоуп по companyId', async () => {
  ...
  expect(findMany).toHaveBeenCalledWith(
    expect.objectContaining({
      where: {
        isActive: true,
        OR: [{ role: { in: ['manager', 'leader'] }, companyId: 'c1' }, { role: 'admin' }],
      },
    })
  );
});
  ...
  expect(findMany).toHaveBeenCalledWith(
    expect.objectContaining({
      where: {
        isActive: true,
        OR: [
          { role: { in: ['manager', 'leader'] }, companyId: NO_COMPANY_SENTINEL },
          { role: 'admin' },
        ],
      },
    })
