...
vi.mock('@/server-actions/inbound', () => ({
  ...
  CHANNEL_TO_CONTACT_TYPE: { telegram: 'telegram', max: 'max', whatsapp: 'whatsapp', email: 'email' }
}));
vi.mock('@/lib/db/prisma', () => ({ prisma: { inboundMessage: { findUnique: inboundMessageFindUnique } } }));
vi.mock('@/lib/featureFlags', () => ({ notFoundIfDisabled }));

import { bindCallAction, createContactFromCallAction, createContactFromInboundAction } from '@/server-actions/contacts';
...
describe('contacts server-actions', () => {
  ...
  it('createContactFromCallAction surfaces a createContact failure without attempting the bind', async () => {
    ...
    const r = await createContactFromCallAction({ callId: 'call1', organizationId: 'o1', name: '', phone: '+79990001122' });
  ...
  it('bindCallAction delegates to bindCall service', async () => {
    ...
    expect(bindCall).toHaveBeenCalledWith(expect.anything(), { sub: 'm1', role: 'manager', companyId: 'c1' }, { callId: 'call1', organizationId: 'o1', contactId: 'k1' });
  });
  ...
  it('createContactFromCallAction creates a contact then binds the call to it', async () => {
    ...
    expect(createContact).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.objectContaining({ name: 'Иван', organizationId: 'o1', channels: [{ type: 'phone', value: '+79990001122' }] }));
    expect(bindCall).toHaveBeenCalledWith(expect.anything(), expect.anything(), { callId: 'call1', organizationId: 'o1', contactId: 'k9' });
  });
  ...
  it('createContactFromCallAction surfaces a bind failure (call not_found) even though createContact succeeded', async () => {
    ...
    expect(bindCall).toHaveBeenCalledWith(expect.anything(), expect.anything(), { callId: 'gone', organizationId: 'o1', contactId: 'k9' });
  });
  ...
  describe('createContactFromInboundAction', () => {
    ...
    it('creates a contact from the sender identity then binds the inbound message to it', async () => {
      ...
      const r = await createContactFromInboundAction({ inboundMessageId: 'im-1', organizationId: 'o1', name: 'Иван' });
      ...
      expect(createContact).toHaveBeenCalledWith(
        expect.anything(),
        expect.anything(),
        expect.objectContaining({ name: 'Иван', organizationId: 'o1', channels: [{ type: 'telegram', value: 'chat-9' }] })
      );
    ...
    it('returns not_found when the inbound message does not exist', async () => {
      ...
      const r = await createContactFromInboundAction({ inboundMessageId: 'gone', organizationId: 'o1', name: 'Иван' });
