...
describe('полнота матрицы', () => {
  ...
  const DESCRIBED_PAGE_GUARDS = new Set([
    ...
    // `У-99`: администратор ИЛИ руководитель — ставку комиссии по организации
    // ведут оба. Рядовой менеджер сюда не проходит, партнёр и заказчик тем
    // более. Границу компании руководителя гард не проверяет — это делает
    // вызывающий (C8), см. `setOrgRateOverrideAction`.
