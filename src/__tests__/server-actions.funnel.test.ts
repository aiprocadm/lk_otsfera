...
const { requireSession, revalidatePath, moveFunnelLead, createFunnelStage, updateFunnelStage, deleteFunnelStage } =
  vi.hoisted(() => ({
...
vi.mock('@/lib/services/access/funnelStages', () => ({ createFunnelStage, updateFunnelStage, deleteFunnelStage }));
...
describe('moveFunnelLeadAction', () => {
  it('calls service, revalidates both cabinets on success', async () => {
    ...
    expect(moveFunnelLead).toHaveBeenCalledWith({}, SESSION, { leadId: 'l1', toStageId: 'default:in_review', reason: undefined });
  ...
  it('missing ids → not_found, no service call', async () => {
    expect(await moveFunnelLeadAction(form({ leadId: 'l1' }))).toEqual({ ok: false, error: 'not_found' });
  ...
  it('passes reason; maps service error', async () => {
    ...
    const res = await moveFunnelLeadAction(form({ leadId: 'l1', toStageId: 'default:rejected', reason: 'x' }));
    expect(res).toEqual({ ok: false, error: 'reason_required' });
    expect(moveFunnelLead).toHaveBeenCalledWith({}, SESSION, { leadId: 'l1', toStageId: 'default:rejected', reason: 'x' });
...
describe('stage CRUD actions', () => {
  it('create builds input, returns id', async () => {
    ...
    const res = await createFunnelStageAction(form({ name: 'Оплата', position: '3', statusAnchor: 'qualified', isTerminal: 'on' }));
    expect(res).toEqual({ ok: true, id: 's1' });
    expect(createFunnelStage).toHaveBeenCalledWith({}, SESSION, expect.objectContaining({ name: 'Оплата', position: 3, statusAnchor: 'qualified', isTerminal: true }));
  });
  it('create maps position_taken', async () => {
    createFunnelStage.mockResolvedValue({ ok: false, error: 'position_taken' });
    expect(await createFunnelStageAction(form({ name: 'x', position: '1', statusAnchor: 'new' }))).toEqual({ ok: false, error: 'position_taken' });
  });
  it('update requires id', async () => {
    expect(await updateFunnelStageAction(form({ name: 'x' }))).toEqual({ ok: false, error: 'validation' });
  ...
  it('update with id calls service', async () => {
    ...
    const res = await updateFunnelStageAction(form({ id: 's9', name: 'x', position: '2', statusAnchor: 'in_review' }));
    expect(res).toEqual({ ok: true });
    expect(updateFunnelStage).toHaveBeenCalledWith({}, SESSION, 's9', expect.objectContaining({ position: 2 }));
  });
  it('delete requires id; with id calls service', async () => {
    expect(await deleteFunnelStageAction(new FormData())).toEqual({ ok: false, error: 'validation' });
