...
describe('POST /api/manager/documents/order-less', () => {
  ...
  it('200 with partner counterpartyType', async () => {
    ...
    const res = await POST(form({ counterpartyType: 'partner', counterpartyId: 'p1', docType: 'other' }));
    expect(res.status).toBe(200);
  });

  it('400 on missing counterpartyId (empty string)', async () => {
    const res = await POST(form({ counterpartyType: 'organization', counterpartyId: '', docType: 'other' }));
    expect(res.status).toBe(400);
  });
  ...
  it('400 when formData() fails (catch branch)', async () => {
    const badReq = { formData: async () => { throw new Error('parse failed'); } } as unknown as Request;
