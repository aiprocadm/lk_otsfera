...
const { parseEnrollmentImportAction } = vi.hoisted(() => ({ parseEnrollmentImportAction: vi.fn() }));
...
describe('EnrollmentWizard — импорт из Excel (шаг 2)', () => {
  ...
  it('успешный импорт: строки добавлены, счётчик обновлён, toast.success, файл ушёл в action', async () => {
    parseEnrollmentImportAction.mockResolvedValue({
      ok: true,
      items: [importedItem(), importedItem({ fullName: 'Олег Смирнов', email: 'oleg@example.com', position: 'инженер' })],
      errors: [],
      warnings: []
    });
    goToStep2();
  ...
  it('ok:true с errors и warnings: валидные строки добавлены, оба списка на экране', async () => {
    ...
    expect(screen.getByRole('status').textContent).toContain('Строка 4: дубликат внутри файла — пропущена');
  ...
  it('дубликат против уже набранной строки: предупреждение «уже в заявке», строка не добавлена', async () => {
    ...
    expect(status.textContent).toContain('Мария Дубль (maria@example.com): уже в заявке — пропущен');
