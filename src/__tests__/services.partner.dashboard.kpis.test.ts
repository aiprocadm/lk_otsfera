...
beforeAll(async () => {
  ...
  const orders = [
    ...
    {
      title: 'Завершённая, оплачена в этом месяце',
      totalAmount: 200000,
      paidAmount: 200000,
  ...
  for (const od of orders) {
    const order = await prisma.order.create({
      data: { ...od, companyId: company.id, partnerId, organizationId: org.id },
    });
