...
describe('ManagerCalendarPage', () => {
  ...
  it('renders the calendar for the current month when the flag is enabled', async () => {
    ...
    const { container } = await renderServerComponent(ManagerCalendarPage(pageProps()));
    ...
    expect(props.calendarHref).toBe('/manager/calendar');
    expect(props.tasksHref).toBe('/manager/tasks');
  });
  ...
  it('passes ?m=2026-08 through to the fetch range and the month prop', async () => {
    ...
    await renderServerComponent(ManagerCalendarPage(pageProps('2026-08')));
  ...
  it('an invalid ?m falls back to the current month', async () => {
    isFeatureEnabled.mockReturnValue(true);
    requireManager.mockResolvedValue(SESSION);
    listCalendarItems.mockResolvedValue([]);
    getEventFormOptions.mockResolvedValue({ users: [], organizations: [], orders: [] });

    await renderServerComponent(ManagerCalendarPage(pageProps('garbage')));

    const expectedMonth = normalizeMonthParam(undefined, new Date());
    expect(monthViewSpy.mock.calls[0][0].month).toBe(expectedMonth);
  });
});
