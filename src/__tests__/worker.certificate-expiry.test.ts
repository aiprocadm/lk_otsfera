...
describe('certificate-expiry processor', () => {
  it('создаёт напоминание и не дублирует на повторном прогоне', async () => {
    ...
    const reminders = await prisma.certificateReminder.findMany({
      where: { certificateId: ids.cert },
    });
