...
describe('assertAllowedStudentPortalUrl', () => {
  ...
  it('accepts a custom allowlist hostname', () => {
    const result = assertAllowedStudentPortalUrl('https://custom.example.com/portal', {
  ...
  it('is case-insensitive for the hostname (normalises to lower-case)', () => {
    const result = assertAllowedStudentPortalUrl(new URL('https://OTSFERA.CDOPROF.COM/test'));
    expect(result.hostname).toBe('otsfera.cdoprof.com');
  });

  // ── failure paths ─────────────────────────────────────────────────────────
  it('throws for non-https protocol', () => {
    expect(() => assertAllowedStudentPortalUrl('http://otsfera.cdoprof.com/path')).toThrow(
      'Student portal URL must use HTTPS protocol.'
    );
  });

  it('throws for a hostname not in the default allowlist', () => {
    expect(() => assertAllowedStudentPortalUrl('https://evil.example.com/redirect')).toThrow(
  ...
  it('handles trailing whitespace in allowlist entries (normalised)', () => {
    const result = assertAllowedStudentPortalUrl('https://clean.example.com/x', {
