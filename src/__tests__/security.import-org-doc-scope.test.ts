...
import { orgInScope, mayCreateOrg, upsertOrgRecord, upsertDocumentRecord } from '@/lib/services/oneCSync/writers';
...
const plainMgrA = { sub: 'm', role: 'manager', companyId: 'companyA', managedOrgIds: ['orgA'] } as unknown as SessionPayload;

const companyScope = importScope(leaderA); // { kind: 'company', companyId: 'companyA' }
const globalScope = importScope(adminA); // { kind: 'global' }

const orgDto = { externalId: 'ORG-1', name: 'Acme', inn: '77', kpp: '01', partnerExternalId: 'acme-partner', updatedAt: '2026-04-01T00:00:00Z' } as never;
const docDto = { externalId: 'D-1', orderExternalId: 'O-1', type: 'contract', name: 'Договор', mimeType: 'application/pdf', size: 10, downloadUrl: 'https://1c/d1', updatedAt: '2026-04-01T00:00:00Z' } as never;
...
function orgDb(over: Record<string, unknown> = {}) {
  return {
    ...
    $transaction: vi.fn(async (cb: (tx: unknown) => unknown) => cb({ company: { create: vi.fn().mockResolvedValue({ id: 'co-new' }) }, organization: { create: vi.fn() } })),
    ...over,
  } as never;
}
function docDb(over: Record<string, unknown> = {}) {
  return {
    order: { findUnique: vi.fn().mockResolvedValue({ id: 'ord1', organizationId: 'orgA', companyId: 'companyA', orderNumber: 'O-1', title: 't' }) },
...
beforeEach(() => {
  ...
  notifyOrgUsers.mockReset(); notifyManagers.mockReset();
...
describe('C8: upsertOrgRecord honours the company floor', () => {
  it('leader CANNOT update an org in ANOTHER company (skip out_of_scope, no write)', async () => {
    const d = orgDb({ organization: { findUnique: vi.fn().mockResolvedValue({ id: 'orgB', companyId: 'companyB', externalId: 'ORG-1' }), findFirst: vi.fn().mockResolvedValue(null), update: vi.fn() } });
    const sum = emptySummary();
    await upsertOrgRecord(d, orgDto, sum, { mode: 'live', notify: true, scope: companyScope });
    expect((d as { organization: { update: ReturnType<typeof vi.fn> } }).organization.update).not.toHaveBeenCalled();
  ...
  it('leader CAN update an org in their OWN company', async () => {
    const d = orgDb({ organization: { findUnique: vi.fn().mockResolvedValue({ id: 'orgA', companyId: 'companyA', externalId: 'ORG-1' }), findFirst: vi.fn().mockResolvedValue(null), update: vi.fn() } });
    const sum = emptySummary();
    await upsertOrgRecord(d, orgDto, sum, { mode: 'live', notify: true, scope: companyScope });
    expect((d as { organization: { update: ReturnType<typeof vi.fn> } }).organization.update).toHaveBeenCalled();
...
describe('C8: upsertDocumentRecord honours the company floor (by the order’s companyId)', () => {
  it('leader CANNOT attach a document to another company’s order (skip out_of_scope, no fetch/create)', async () => {
    const d = docDb({ order: { findUnique: vi.fn().mockResolvedValue({ id: 'ord1', organizationId: 'orgB', companyId: 'companyB', orderNumber: 'O-1', title: 't' }) } });
    const sum = emptySummary();
    ...
    expect((d as { document: { create: ReturnType<typeof vi.fn> } }).document.create).not.toHaveBeenCalled();
    expect(sum.skipped).toBe(1);
    expect(sum.skips[0]).toMatchObject({ reason: 'out_of_scope' });
  });
  ...
  it('leader CAN attach a document to an order in their OWN company', async () => {
    ...
    const sum = emptySummary();
    await upsertDocumentRecord(d, docDto, sum, { mode: 'live', notify: false, scope: companyScope });
    expect(fetchAndStore1CDocument).toHaveBeenCalled();
    expect((d as { document: { create: ReturnType<typeof vi.fn> } }).document.create).toHaveBeenCalled();
    expect(sum.created).toBe(1);
  });
});
