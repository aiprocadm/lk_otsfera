...
describe('ManagerMessagesInbox', () => {
  it('empty: renders empty-state', () => {
    const html = renderToString(React.createElement(ManagerMessagesInbox, { rows: [], nextCursor: null }));
  ...
  it('author with no name: falls back to email for display + initials', () => {
    const rows = [makeItem({ author: { role: 'organization', name: null, email: 'nikita@example.com' } })];
    const html = renderToString(React.createElement(ManagerMessagesInbox, { rows, nextCursor: null }));
  ...
  it('single-word name: initials fall back to first two chars', () => {
    const rows = [makeItem({ author: { role: 'organization', name: 'Мадонна', email: 'm@x.com' } })];
    const html = renderToString(React.createElement(ManagerMessagesInbox, { rows, nextCursor: null }));
    expect(html).toContain('МА');
  });

  it('two-word name: initials use first letter of each word', () => {
    const rows = [makeItem({ author: { role: 'organization', name: 'Анна Иванова', email: 'a@x.com' } })];
    const html = renderToString(React.createElement(ManagerMessagesInbox, { rows, nextCursor: null }));
  ...
  it('renders "Дальше" link with cursor when nextCursor is set', () => {
    const rows = [makeItem({})];
    const html = renderToString(React.createElement(ManagerMessagesInbox, { rows, nextCursor: 'cur1' }));
