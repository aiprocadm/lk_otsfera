...
// Разделы этапов закрыты поведенческими флагами: «Миграция из Битрикс24» —
// `bitrix_migration`, «Шаблоны ответов» и «Сайт» — `comm_center` (этап 3,
// `У-217`). Для матрицы ПРАВ считаем их включёнными: иначе раздел выпадает из
// выборки и тест проверяет не права, а флаг. Регистрируется ПОСЛЕ сброса
// окружения выше, чтобы тот его не затирал.
beforeEach(() => {
