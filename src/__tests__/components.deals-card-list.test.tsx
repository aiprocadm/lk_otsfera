...
describe('DealsCardList', () => {
  ...
  it('omits the order-number prefix when orderNumber is null', () => {
    const html = renderToString(React.createElement(DealsCardList, { rows: [makeRow({ orderNumber: null })] }));
    expect(html).not.toContain('№ <!-- -->');
  });

  it('shows "Без долга" (gray) when debt is 0', () => {
    const html = renderToString(React.createElement(DealsCardList, { rows: [makeRow({ debt: '0.00' })] }));
  ...
  it('omits the deadline row when deadline is null', () => {
    const html = renderToString(React.createElement(DealsCardList, { rows: [makeRow({ deadline: null })] }));
