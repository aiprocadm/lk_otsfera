...
describe('GET /api/client-requests', () => {
  ...
  it('200: результат сервиса отдаётся как есть', async () => {
    vi.mocked(getSession).mockResolvedValue(manager);
    vi.mocked(listClientRequests).mockResolvedValue({
      rows: [{ id: 'R1' }],
      nextCursor: null,
    } as never);
    ...
    expect(vi.mocked(listClientRequests)).toHaveBeenCalledWith({}, manager, {
      status: undefined,
      cursor: undefined,
    });
  });
  ...
  it('неизвестный status игнорируется (status: undefined)', async () => {
    ...
    expect(vi.mocked(listClientRequests)).toHaveBeenCalledWith(
      {},
      manager,
...
describe('GET /api/client-requests/[id]', () => {
  ...
  it('200 → {request}', async () => {
    vi.mocked(getSession).mockResolvedValue(partner);
    vi.mocked(getClientRequest).mockResolvedValue({
      ok: true,
      request: { id: 'R1', subject: 'Обучение' },
    } as never);
...
describe('PATCH /api/client-requests/[id]', () => {
  ...
  it('takeInTriage: not_found → 404', async () => {
    ...
    expect((await PATCH(jsonReq({ action: 'takeInTriage' }, 'PATCH'), ctx('R404'))).status).toBe(
      404
    );
  });
  ...
  it('takeInTriage: 200 → {request}', async () => {
    vi.mocked(getSession).mockResolvedValue(manager);
    vi.mocked(takeInTriage).mockResolvedValue({
      ok: true,
      request: { id: 'R1', status: 'in_triage' },
    } as never);
    const res = await PATCH(jsonReq({ action: 'takeInTriage' }, 'PATCH'), ctx('R1'));
    expect(res.status).toBe(200);
  ...
  it('reject: 200, причина прокинута в сервис', async () => {
    vi.mocked(getSession).mockResolvedValue(manager);
    vi.mocked(rejectClientRequest).mockResolvedValue({
      ok: true,
      request: { id: 'R1', status: 'rejected' },
    } as never);
    const res = await PATCH(jsonReq({ action: 'reject', reason: 'Дубль' }, 'PATCH'), ctx('R1'));
    expect(res.status).toBe(200);
    expect(vi.mocked(rejectClientRequest)).toHaveBeenCalledWith({}, manager, {
      id: 'R1',
      reason: 'Дубль',
    });
  });
  ...
  it('reject: validation → 400', async () => {
    ...
    expect(
      (await PATCH(jsonReq({ action: 'reject', reason: '' }, 'PATCH'), ctx('R1'))).status
    ).toBe(400);
  });
