...
beforeEach(() => {
  ...
  orgFinance.getOrgFinanceKpisForOrgs.mockImplementation(
    async (_prisma: unknown, ids: string[]) =>
      new Map(ids.map((id) => [id, { billed: '100.00', paid: '40.00', outstanding: '60.00' }]))
  );
  orgFinance.listOrgPaymentsForOrgs.mockImplementation(
    async (_prisma: unknown, ids: string[]) =>
      new Map(
        ids.map((id) => [
          id,
  ...
  orgFinance.getOrgIntermediaryCommissionForOrgs.mockImplementation(
    async (_prisma: unknown, ids: string[]) =>
      new Map(
        ids.map((id) => [id, { effectiveRate: '0.1', totalCommission: '10.00', perOrder: [] }])
      )
  );
});
...
describe('getManagerFinanceOverview', () => {
  it('plain manager: scoped to managedOrgIds, commission NOT computed', async () => {
    ...
    const res = await getManagerFinanceOverview(
      prisma,
      session({ managedOrgIds: ['o1', 'o2'], companyId: 'c1' }),
      { teamMode: false }
    );
  ...
  it('leader with teamMode=ON: company-wide scope + commission computed', async () => {
    ...
    expect(res.sections[0].commission).toEqual({
      effectiveRate: '0.1',
  ...
  it('admin: unscoped (no where) + commission computed', async () => {
    const prisma = fakePrisma([{ id: 'o1', name: 'A' }]);
    const res = await getManagerFinanceOverview(prisma, session({ role: 'admin' }), {
      teamMode: false,
    });
  ...
  it('empty scope → no sections, zero summary', async () => {
    ...
    const res = await getManagerFinanceOverview(
      prisma,
      session({ managedOrgIds: [], companyId: 'c1' }),
      { teamMode: false }
    );
