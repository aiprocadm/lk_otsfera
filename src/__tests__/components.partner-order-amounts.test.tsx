...
describe('OrderAmounts', () => {
  it('renders total/paid/debt tiles with danger tone when debt > 0', () => {
    const html = renderToString(React.createElement(OrderAmounts, { deal: makeDeal() }));
  ...
  it('renders neutral debt tone when debt is 0', () => {
    ...
    const html = renderToString(React.createElement(OrderAmounts, { deal }));
  ...
  it('shows "Без НДС" when vatIncluded is false', () => {
    const html = renderToString(
      React.createElement(OrderAmounts, { deal: makeDeal({ vatIncluded: false, vatRate: null }) })
    );
