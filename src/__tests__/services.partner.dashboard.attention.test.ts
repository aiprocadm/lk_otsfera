...
beforeAll(async () => {
  ...
  const orders = [
    { title: 'Зависшая 20 дней', totalAmount: 50000, paidAmount: 0, executionStatus: 'in_progress' as const, financialStatus: 'billed' as const, updatedAt: twentyDaysAgo, createdAt: twentyDaysAgo },
    { title: 'Свежая', totalAmount: 50000, paidAmount: 0, executionStatus: 'in_progress' as const, financialStatus: 'billed' as const, updatedAt: tenDaysAgo, createdAt: tenDaysAgo },
    { title: 'Просроченный счёт', totalAmount: 50000, paidAmount: 0, executionStatus: 'in_progress' as const, financialStatus: 'billed' as const, deadline: threeDaysAgo, updatedAt: tenDaysAgo }
  ];
  for (const od of orders) {
    const order = await prisma.order.create({ data: { ...od, companyId: c.id, partnerId, organizationId: org.id } });
