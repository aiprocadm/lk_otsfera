 ...
 * должен исполниться catch (отказ С ПРИЧИНОЙ про сеть, `У-213`) И finally
 * (`clearTimeout`).
...
describe('sendMaxMessage — catch→finally', () => {
  it('fetch reject → {ok:false} (catch), clearTimeout в finally не бросает', async () => {
    ...
    await expect(sendMaxMessage('chat-1', 'hi')).resolves.toEqual({
      ok: false,
...
describe('sendTelegramMessage — catch→finally', () => {
  it('fetch reject → {ok:false} (catch), clearTimeout в finally не бросает', async () => {
    ...
    await expect(sendTelegramMessage('123456', 'hi')).resolves.toEqual({
      ok: false,
...
describe('sendWhatsAppMessage — catch→finally', () => {
  it('fetch reject → {ok:false} (catch), clearTimeout в finally не бросает', async () => {
    ...
    await expect(sendWhatsAppMessage('+79991234567', 'hi')).resolves.toEqual({
      ok: false,
