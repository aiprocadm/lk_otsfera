...
describe('У-130: SLA переехал из «Команды» в хаб', () => {
  ...
  it('раздел заведён у администратора и руководителя', () => {
    ...
    // Этап 4 (`У-225`): в разделе появился третий порог — дни просрочки ЗАДАЧИ,
    // поэтому он называется просто «SLA». Второй раздел с похожим названием
    // заставил бы человека гадать, в каком из двух его порог.
    expect(section?.title).toBe('SLA');
  });
