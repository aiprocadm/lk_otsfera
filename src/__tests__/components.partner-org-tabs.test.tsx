...
describe('OrgTabs', () => {
  it('renders all tabs including "Настройки" when isAdmin is true, active tab styled distinctly', () => {
    const html = renderToString(
      React.createElement(OrgTabs, { orgId: 'o1', active: 'employees', isAdmin: true })
    );
  ...
  it('hides "Настройки" when isAdmin is false', () => {
    const html = renderToString(
      React.createElement(OrgTabs, { orgId: 'o1', active: 'employees', isAdmin: false })
    );
  ...
  it('builds the documents tab href as a dedicated route (not a query param)', () => {
    const html = renderToString(
      React.createElement(OrgTabs, { orgId: 'o2', active: 'documents', isAdmin: false })
    );
  ...
  it('builds query-param hrefs for the remaining tabs', () => {
    const html = renderToString(
      React.createElement(OrgTabs, { orgId: 'o3', active: 'comments', isAdmin: false })
    );
  ...
  it('renders inactive tabs with the neutral border/text classes', () => {
    const html = renderToString(
      React.createElement(OrgTabs, { orgId: 'o1', active: 'history', isAdmin: false })
    );
