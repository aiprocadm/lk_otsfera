...
const { requireManager, addDealNote, initiateOutboundCall, notFoundIfDisabled, findUnique } = vi.hoisted(() => ({
...
it('initiateCallAction derives fromInternal from the caller\'s User.internalPhone and delegates', async () => {
  ...
  expect(initiateOutboundCall).toHaveBeenCalledWith(prisma, session, { orderId: 'o1', toNumber: '+70000000000', fromInternal: '101' });
