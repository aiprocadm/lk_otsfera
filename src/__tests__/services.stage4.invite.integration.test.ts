...
describe('этап 4: invite-онбординг (integration, живой Postgres)', () => {
  ...
  it('peekTokenPurpose: invite-токен → {valid:true, purpose:invite} и НЕ гасится (usedAt остаётся null)', async () => {
    ...
    expect(await peekTokenPurpose(prisma, 'no-such-token')).toEqual({ valid: false, purpose: null });
  });

  it('resendInvite (admin): старый токен погашен, новый живой, ссылка другая', async () => {
    const res = await resendInvite(prisma, adminSession, { userId: invitedUserId, sendEmail: false });
    ...
    expect(await peekTokenPurpose(prisma, tokenFromUrl(firstInviteUrl))).toEqual({ valid: false, purpose: null });
    expect(await peekTokenPurpose(prisma, tokenFromUrl(secondInviteUrl))).toEqual({ valid: true, purpose: 'invite' });
  ...
  it('verifyAndConsumeToken по новой ссылке ставит пароль и гасит токен', async () => {
    const consumed = await verifyAndConsumeToken(prisma, tokenFromUrl(secondInviteUrl), 'stage4-int-new-hash');
    ...
    expect(await peekTokenPurpose(prisma, tokenFromUrl(secondInviteUrl))).toEqual({ valid: false, purpose: null });
    expect(await verifyAndConsumeToken(prisma, tokenFromUrl(secondInviteUrl), 'x')).toEqual({ ok: false, reason: 'used' });
  });
