...
describe('twoFactor service (integration)', () => {
  it('create → verify happy path; challenge row is consumed', async () => {
    ...
    expect(await verifyTwoFactorCode(prisma, userId, code)).toEqual({
      ok: true,
      method: 'challenge',
    });
  ...
  it('re-login overwrites the previous challenge (single row per user)', async () => {
    ...
    if (first.code !== second.code) {
      expect(await verifyTwoFactorCode(prisma, userId, first.code)).toEqual({
        ok: false,
        error: 'invalid_code',
      });
    }
  ...
  it('5 wrong attempts kill the challenge', async () => {
    ...
    expect(await verifyTwoFactorCode(prisma, userId, wrong)).toEqual({
      ok: false,
  ...
  it('backup code works once; regeneration invalidates old codes', async () => {
    ...
    expect(await verifyTwoFactorCode(prisma, userId, codes[0]!)).toEqual({
      ok: true,
      method: 'backup',
    });
    ...
    expect(await verifyTwoFactorCode(prisma, userId, codes[1]!)).toEqual({
      ok: false,
      error: 'invalid_code',
    });
    expect(await verifyTwoFactorCode(prisma, userId, fresh[0]!)).toEqual({
      ok: true,
      method: 'backup',
    });
  });
