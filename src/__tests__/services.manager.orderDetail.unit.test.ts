...
describe('loadManagerOrderDetail', () => {
  it('returns null and skips audit/comment loads when the order is not visible', async () => {
    ...
    expect((prisma as never as { auditLog: { findMany: ReturnType<typeof vi.fn> } }).auditLog.findMany).not.toHaveBeenCalled();
    expect((prisma as never as { comment: { findMany: ReturnType<typeof vi.fn> } }).comment.findMany).not.toHaveBeenCalled();
  });
  ...
  it('loads audit + comments and folds per-order context into documentRows', async () => {
    ...
    const comments = [{ id: 'c-1', author: { id: 'u-1', name: 'Иван', email: 'i@x', role: 'manager' } }];
    ...
    const auditCall = (prisma as never as { auditLog: { findMany: ReturnType<typeof vi.fn> } }).auditLog.findMany.mock.calls[0][0];
  ...
  it('returns an empty documentRows array when the order has no documents', async () => {
    getOrder.mockResolvedValue({ id: 'order-2', orderNumber: 'A-2', title: 'Пусто', documents: [] } as never);
    const prisma = fakePrisma();
