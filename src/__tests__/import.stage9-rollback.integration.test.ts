...
describe('этап 9 — откат импорта (живой Postgres)', () => {
  ...
  it('Т-40: батч старше 30 дней — expired и в списке, и на попытке отката', async () => {
    // Список отдаёт 20 самых свежих батчей (`take: 20`). Состаренный на 40
    // дней батч уходит в самый низ, и если база живёт давно, его вытесняет
    // чужой мусор — тест падал на локальном Postgres, оставаясь зелёным на
    // чистом. Осиротевшие батчи (автор удалён) бывают ТОЛЬКО от тестов:
    // на проде пользователей не удаляют, а `importedById` обнуляется лишь
    // при удалении автора. Поэтому убрать их здесь безопасно.
    await prisma.oneCImportBatch.deleteMany({ where: { importedById: null } });
