...
describe('extractVat', () => {
  ...
  /**
   * Страж копейки (хотфикс №34, находка `С-8` от 09.09.2026).
   *
   * Включённый НДС считается В КОПЕЙКАХ. Прежняя формула в рублях —
   * `amount * rate / (100 + rate)`, округлённая через `* 100 / 100`, — на
   * каждом 35-м платеже (перебор 200 000 сумм, ставка 20 %) не дотягивала до
   * половины копейки и уводила результат вниз. Бухгалтер сверяет НДС с 1С по
   * копейке, поэтому «почти верно» тут не годится.
   *
   * Суммы ниже подобраны перебором: на каждой из них рублёвая формула даёт
   * на копейку меньше. `toBe`, а не `toBeCloseTo`, — именно копейка и
   * проверяется.
   */
  it.each([
    [89_323.59, 20, 14_887.27],
    [198_102.03, 20, 33_017.01],
    [1_148_025.93, 20, 191_337.66],
    [1_533_922.17, 20, 255_653.7],
    [3_028_243.59, 20, 504_707.27],
    [4_660_937.43, 20, 776_822.91],
  ])('включённый НДС от %s ₽ при %s %% — ровно %s ₽, без потери копейки', (amount, rate, vat) => {
    expect(extractVat(`в том числе НДС ${rate}%`, amount)).toBe(vat);
  });
});
