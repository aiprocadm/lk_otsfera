...
describe('setLeadStatus', () => {
  ...
  it('forbids new → qualified (must go through in_review)', async () => {
    const d = db({ id: 'L1', status: 'new', partnerId: 'p1', organizationId: 'o1' });
    expect(await setLeadStatus(d, { leadId: 'L1', managerId: 'm1', status: 'qualified' })).toEqual({ ok: false, error: 'lifecycle_violation' });
  });
  it('forbids reaching promoted_to_order via setLeadStatus', async () => {
    ...
    expect(await setLeadStatus(d, { leadId: 'L1', managerId: 'm1', status: 'promoted_to_order' })).toEqual({ ok: false, error: 'lifecycle_violation' });
  });
});
...
describe('rejectLead', () => {
  ...
  it('cannot reject a promoted lead', async () => {
    const d = db({ id: 'L1', status: 'promoted_to_order', partnerId: 'p1', organizationId: 'o1' });
    expect(await rejectLead(d, { leadId: 'L1', managerId: 'm1', reason: 'x' })).toEqual({ ok: false, error: 'lifecycle_violation' });
  });
});
