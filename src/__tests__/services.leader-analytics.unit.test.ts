...
describe('getFunnelAnalytics', () => {
  ...
  it('snapshot buckets open leads into default stages with Decimal-safe sums; empty stage → 0/"0.00"', async () => {
    ...
    expect(res.snapshot).toEqual([
      ...
      { stageId: 'default:in_review', name: 'В работе', color: null, count: 1, estimatedSum: '500.50' },
      { stageId: 'default:qualified', name: 'Квалифицирован', color: null, count: 0, estimatedSum: '0.00' }
    ]);
  });
  ...
  it('cohort math: 5 leads → 2 promoted / 1 rejected / 2 in work, conversion 40%, rejected 20%, avg days, per-manager + unassigned rows', async () => {
    listCompanyManagers.mockResolvedValue([
      ...
      { id: 'm3', name: 'Сидоров', email: 's@x.ru', isActive: true, managerRole: null, assignments: [] }
    ]);
    const cohort = [
      {
    ...
    expect(res.perManager).toEqual([
      ...
      { managerId: null, name: 'Без менеджера', assigned: 1, promoted: 0, rejected: 0, conversionPct: 0 }
    ]);
  });
...
describe('getPlanFact', () => {
  ...
  it('no unassigned facts and no unassigned completed orders → "Без менеджера" row absent', async () => {
    ...
    const payment = { findMany: vi.fn().mockResolvedValue([{ amount: D('500'), isRefund: false, order: { managerId: 'm1' } }]) };
...
describe('upsertSalesTarget', () => {
  ...
  it('admin → ok (valid upsert)', async () => {
    ...
    expect(call.where).toEqual({ companyId_managerId_year_month: { companyId: 'c1', managerId: 'm1', year: 2026, month: 7 } });
  ...
  it('leader manager (managerRole=leader) → ok', async () => {
    const session = { sub: 'leader2', role: 'manager', companyId: 'c1', managerRole: 'leader' } as never;
    const user = { findUnique: vi.fn().mockResolvedValue({ id: 'm1', role: 'manager', companyId: 'c1' }) };
    const prisma = fakePrisma({ user });
  ...
  it('foreign-company manager → forbidden', async () => {
    const user = { findUnique: vi.fn().mockResolvedValue({ id: 'm1', role: 'manager', companyId: 'other-co' }) };
    const prisma = fakePrisma({ user });
    const res = await upsertSalesTarget(prisma, leaderSession, args);
    expect(res).toEqual({ ok: false, error: 'forbidden' });
  });
  ...
  it('target user with role=organization → not_found', async () => {
    const user = { findUnique: vi.fn().mockResolvedValue({ id: 'm1', role: 'organization', companyId: 'c1' }) };
    const prisma = fakePrisma({ user });
    const res = await upsertSalesTarget(prisma, leaderSession, args);
    expect(res).toEqual({ ok: false, error: 'not_found' });
  });
