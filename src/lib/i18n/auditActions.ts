/**
 * ВНИМАНИЕ: это НЕ общий словарь журнала аудита. Общий (существительные:
 * «Создание пользователя») живёт в `lib/audit/labels` и покрывает весь реестр
 * AUDIT_ACTIONS. Здесь — глагольные фразы от лица актора для ленты дашборда
 * («Иван утвердил отчёт»); грамматика другая, объединять их нельзя.
 *
