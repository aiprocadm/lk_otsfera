...
export const SETTINGS_SECTIONS: readonly SettingsSection[] = [
  ...
  {
    // Спека 2026-09-12 (Р-М-6): мессенджеры подключаются как каналы переписки
    // с клиентами — свой раздел, а не три формы среди восьми на обзоре. Только
    // администратор: здесь секреты платформы (`Р-22`), руководителю остаётся
    // светофор на обзоре.
    id: 'integrations.messengers',
    group: 'integrations',
    title: 'Подключение мессенджеров',
    description: 'Telegram, MAX и WhatsApp: токены ботов, вебхуки, проверка связи, включение.',
    icon: '📱',
    path: 'integrations/messengers',
    capability: 'settings.integrations.manage',
    cabinets: ['admin'],
    // Старого адреса нет: формы жили на обзоре, а не на своём маршруте.
    legacyHrefs: [],
  },
  {
  ...
  },
  {
    ...
    // `У-225` (этап 4): здесь же порог просрочки ЗАДАЧ, поэтому раздел
    // называется просто «SLA». Заводить второй раздел с похожим названием
    // значило бы заставить человека гадать, в каком из двух его порог.
    id: 'catalogs.slaIntake',
    group: 'catalogs',
    title: 'SLA',
    description: 'Через сколько подсветить, эскалировать и напомнить о просроченной задаче.',
    icon: '⏱️',
  ...
  },
  {
    ...
    description: 'Свои формулировки для договора, доп. соглашения и коммерческого предложения.',
