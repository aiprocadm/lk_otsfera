...
const productionEnvSchema = z
  .object({
    ...
    /**
     * `У-132` (дефект `Д-36`): мастер-ключ секретов интеграций. Без него всё,
     * что вводится в интерфейсе как секрет (токены ботов, ключ почты, пароль
     * IMAP), сохранить нельзя — а узнавал об этом человек только в момент
     * нажатия «Сохранить». В production ключ обязателен: сервер, на котором
     * интеграции нельзя настроить, не поднимается.
     *
     * Минимум 16 — та же граница, что в `resolveKey()`; шире её ставить
     * нельзя, иначе схема и рантайм разойдутся.
     */
    APP_ENCRYPTION_KEY: requiredSecret(16),
