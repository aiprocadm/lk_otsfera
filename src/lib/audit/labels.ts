...
const ACTION_LABELS: Record<AuditAction, string> = {
  ...
  // `У-149`, `У-159`: каждая отправка документа клиенту — своё событие,
  // включая повторную: «когда клиенту в последний раз отправляли счёт» —
  // обычный вопрос, и ответ на него должен быть в журнале.
  document_sent: 'Отправка документа заказчику',
  ...
  order_line_removed: 'Удаление строки заказа',
  order_lines_built_from_items: 'Сборка строк из позиций заказа',
  order_total_set_manually: 'Сумма заказа задана вручную',
...
const ENTITY_LABELS: Record<AuditEntity, string> = {
  ...
  messenger_dialog: 'Диалог в мессенджере',
  ...
  document_template: 'Текст договора',
