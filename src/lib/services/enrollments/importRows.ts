...
export async function parseEnrollmentImportWorkbook(buffer: Buffer | ArrayBuffer): Promise<EnrollmentImportResult> {
  let wb: ExcelJS.Workbook;
  try {
    wb = await loadXlsxWorkbook(buffer);
  } catch {
    return { ok: false, errors: ['Не удалось прочитать файл — ожидается файл Excel (.xlsx). Скачайте шаблон.'] };
  }
  const ws = wb.worksheets[0];
  if (!ws) return { ok: false, errors: ['В файле нет ни одного листа. Скачайте шаблон.'] };

  const colIndex = new Map<string, number>();
  ws.getRow(1).eachCell((cell, col) => colIndex.set(normalizeHeader(cellToString(cell.value)), col));
  ...
  if (!fieldToCol.has('fullName') || !fieldToCol.has('email')) {
    return { ok: false, errors: ['В первой строке файла не найдены колонки «ФИО» и «Email». Скачайте шаблон.'] };
  }
  ...
  if (!items.length && !errors.length && !warnings.length) {
    return { ok: false, errors: ['В файле нет ни одной строки со слушателями (заполняются строки со 2-й).'] };
  }
