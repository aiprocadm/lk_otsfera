...
function snapshotLines(lines: PrintLineInput[]) {
  return lines.map((line, index) => {
    ...
    return {
      ...
      /**
       * `У-164`: цена строки указана С НДС или БЕЗ. Признак ОБЯЗАН попасть в
       * снимок, а не остаться значением по умолчанию колонки.
       *
       * Колонка заведена ради переноса строк принятого предложения в заказ, и
       * восстановить признак из суммы налога нельзя: при ставке 0 и при «не
       * облагается» результат неразличим. Без этой строки печать была бы
       * верной (она считает по форме), а снимок — врал: заказ по предложению
       * с ценами «без НДС» разошёлся бы с ним ровно на ставку налога.
       */
