...
export async function attention(
  ...
  const [overdueOrders, recentOrgComments, unsignedActs, stalledOrders] = await Promise.all([
    prisma.order.findMany({
      where: {
        AND: [scope, { deadline: { lt: now } }, { executionStatus: { notIn: [...TERMINAL_EXEC] } }],
      },
    ...
    prisma.order.findMany({
      where: {
        AND: [scope, { executionStatus: 'in_progress' }, { updatedAt: { lt: fourteenDaysAgo } }],
      },
  ...
  const candidateOrderIds = Array.from(new Set(recentOrgComments.map((c) => c.orderId)));
