...
export type GetDealActivityResult =
  { ok: true; items: ActivityItem[] } | { ok: false; error: 'not_found' };
...
export async function getDealActivity(
  ...
  const items: ActivityItem[] = [
    ...
    ...inbound.map((i): ActivityItem => ({
      kind: 'message_in',
      id: i.id,
      at: i.sentAt ?? i.createdAt,
      channel: i.channel,
      sender: i.senderDisplay ?? i.senderRef,
      body: i.body,
    ...
    ...calls.map((c): ActivityItem => ({
      ...
      recordingReady: c.recordingScanStatus === 'clean' && !!c.recordingPath,
      initiator: c.initiatedBy?.name ?? null,
    })),
    ...notes.map((n): ActivityItem => ({
      kind: 'note',
      id: n.id,
      at: n.createdAt,
  ...
  if (inbound.length)
    piiArgs.push({
      session,
      context: 'deal_activity_inbound',
      subjectIds: inbound.map((i) => i.id),
    });
  if (calls.length)
    piiArgs.push({ session, context: 'deal_activity_calls', subjectIds: calls.map((c) => c.id) });
  if (piiArgs.length) await recordPiiAccessMany(prisma, piiArgs);

  const filtered =
    opts.view === 'dialogue' ? items.filter((i) => DIALOGUE_KINDS.has(i.kind)) : items;
