...
/** Синхронизирует множество исполнителей задачи (set-разница). */
...
export async function updateTask(
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (data.assigneeIds !== undefined) await syncAssignees(tx, id, data.assigneeIds);
      await recordAudit(tx, {
...
export async function assignTask(
  ...
  try {
    await prisma.$transaction(async (tx) => {
      const before = await tx.task.findUnique({ where: { id: args.taskId }, select: SCOPE_SELECT });
      if (!before) throw new TaskError('not_found');
      if (!canSeeTask(session, scopeArg(before))) throw new TaskError('not_found');
      ...
      await syncAssignees(tx, args.taskId, args.assigneeIds);
      await recordAudit(tx, {
