...
export type ContactListScope = 'all' | 'with_org' | 'without_org' | 'archived';
export type ContactListSort = 'name' | 'updated';
...
export async function listContacts(
  ...
  const [rows, total] = await Promise.all([
    prisma.contact.findMany({
      where,
      select: LIST_SELECT,
      // Хвост `id` — устойчивая постраничность (страж `С-8`): одинаковые имена
      // и даты у пачки контактов не должны «дышать» на границах страниц.
      orderBy:
