    ...
    try {
      await appendInboundToDialog(prisma, {
        ...
        ...(dto.attachmentPath && dto.attachmentName && dto.attachmentMime
          ...
          : {}),
        // Привязка диалога. Кроме точного совпадения по адресу сюда входит
        // случай, когда отправитель ИЗВЕСТЕН ЗАРАНЕЕ — его передал вызывающий
        // (`dto.sender`), как делает вопрос из кабинета (`У-212`): там человек
        // сидит в своём кабинете под своей учётной записью, гадать не о чем.
        //
        // Без этой ветки диалог создавался БЕЗ компании, а ничейный диалог по
        // правилу общей очереди видят сотрудники ЛЮБОЙ компании — то есть
        // вопрос клиента одного учебного центра читал бы другой. Само письмо
        // при этом скоупилось правильно, и расхождение было незаметным.
        // Заодно чинится тишина: уведомление о новом обращении требует
        // ответственного или организации, а пустыми были оба.
        // Компания обязательна: без неё привязывать не к чему, и диалог
        // честно остаётся в общей очереди (так бывает у пользователя, ещё не
        // прикреплённого к организации).
        binding: dto.sender?.companyId
          ? {
              companyId: dto.sender.companyId,
              organizationId: dto.sender.organizationId,
              contactId: null,
