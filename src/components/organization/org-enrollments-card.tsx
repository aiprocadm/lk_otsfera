...
/**
 * Дашборд-блок «Заявки на обучение» организации (этап 2 PR-2, ФТ-2.4):
 * кнопка подачи + последние заявки со статус-бейджами и ссылками на деталку.
 */
export function OrgEnrollmentsCard({ rows }: { rows: OrgEnrollmentSummary[] }) {
  return (
    <div className='bg-white border border-gray-200 rounded-xl p-5'>
      ...
      {rows.length === 0 ? (
        <p className='text-sm text-gray-500'>
          Заявок пока нет — подайте первую: направление из справочника, слушатели списком или из Excel.
        </p>
      ) : (
        <ul className='space-y-2 text-sm'>
          {rows.map((r) => (
            <li key={r.id} className='flex items-center justify-between gap-3'>
              <Link
                href={`/organization/enrollments/${r.id}`}
              ...
              <span className='text-gray-400 text-xs whitespace-nowrap'>{fmtDate(r.createdAt)}</span>
            </li>
          ))}
        </ul>
      )}
