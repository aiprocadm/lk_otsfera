...
export async function ManagersBlock({ orgId }: { orgId: string }) {
  ...
  return (
    <section className='bg-white border border-gray-200 rounded-xl p-5 shadow-sm'>
      <div className='flex items-start justify-between gap-3 mb-3'>
        <div>
          ...
          <p className='text-xs text-gray-500 mt-0.5'>
            Менеджеры Промтехносферы, которые видят заказы этой организации в
            своём кабинете.
          </p>
        </div>
      ...
      {inactive.length > 0 && (
        <>
          <h3 className='text-xs uppercase tracking-wide text-gray-400 mt-4 mb-1'>
            Архив
          </h3>
          <ul className='divide-y divide-gray-100 -mx-5'>
            {inactive.map((a) => (
