...
export function QueueStatsGrid({ rows }: { rows: QueueStatsRow[] }) {
  return (
    <div>
      ...
      <CardList>
        {rows.map((row) => (
          <Card key={row.queue} title={<span className="font-mono text-xs">{row.queue}</span>}>
            <CardRow label="Ожидают">{row.counts.waiting}</CardRow>
            <CardRow label="Активные">{row.counts.active}</CardRow>
            <CardRow label="Отложены">{row.counts.delayed}</CardRow>
            <CardRow label="Завершено">{row.counts.completed}</CardRow>
            <CardRow label="Сбоев">
              <span
                className={`inline-block px-2 py-0.5 rounded text-xs ${badgeClass(row.counts.failed, 'failed')}`}
              >
                {row.counts.failed}
              </span>
            </CardRow>
            ...
            {rows.map((row) => (
              <tr key={row.queue} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3 text-right tabular-nums">
                  <span
                    className={`inline-block px-2 py-0.5 rounded text-xs ${badgeClass(row.counts.active, 'active')}`}
                  >
                    {row.counts.active}
                  </span>
                </td>
