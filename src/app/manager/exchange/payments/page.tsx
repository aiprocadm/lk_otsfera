...
/**
 * Вкладка «Выписка по счёту 51» раздела «Обмен с 1С» (`У-113`).
 *
 ...
 */
const BASE_PATH = '/manager/exchange/payments';

export default async function ManagerExchangePaymentsPage({
  searchParams,
}: {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
}) {
  const session = await requireManager();
  // Руководитель ведёт обмен в своём хабе настроек (`У-118`). Фильтры и
  // страница очереди переносятся туда как есть.
  ...
  return (
    <div className="space-y-6">
      <div>
        <PageHeader
          title="Выписка по счёту 51"
          subtitle="Загрузите выгрузку 1С «Карточка счёта 51» — банковскую выписку по расчётному счёту. Оплаты клиентов привязываются к заказам; непонятные строки попадают в очередь разбора ниже."
        />
      </div>
