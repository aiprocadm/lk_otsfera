...
export async function GET(req: Request) {
  ...
  if (session.role !== 'organization') return NextResponse.json({ error: 'forbidden' }, { status: 403 });
  ...
  const buf = await renderCertificatesXlsx({ rows: res.certificates, total: res.total, showOrganization: false });
