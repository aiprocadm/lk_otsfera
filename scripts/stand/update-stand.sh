...
#
# У стенда своя копия репозитория, своя база, своя очередь и свой бакет файлов —
# с разработкой он не пересекается ничем.
#
...
# Миграции — ПОСЛЕ успешной сборки: незачем трогать схему ради кода, который
# даже не собрался. База у стенда СВОЯ (lk_otsfera_stand), очередь — своя
# (номер базы Redis), файлы — в своём бакете; разработку это не заденет.
#
# ВАЖНО про пользователя БД. Приложение ходит в базу под ограниченной ролью
# (`lk_otsfera_stand_app`), и это правильно. Но часть миграций требует ПРАВ
# ВЛАДЕЛЬЦА: `ALTER TYPE ... ADD VALUE` и `DROP TYPE` падают под чужой ролью с
# `42501: must be owner of type`. Ровно так стенд простоял сутки на старой
# версии 29–30.07.2026: миграция этапа 1 не применилась, скрипт честно
# откатывался, а в логе это выглядело как «миграции сломаны» — хотя локально
# они проходили (там разработчик и есть владелец схемы).
#
# Поэтому миграции применяем под отдельным доступом MIGRATE_DATABASE_URL
# (владелец схемы). Если он не задан — падаем сразу с понятным текстом, а не
# ловим 42501 на первой же enum-миграции.
if [[ "$do_migrate" -eq 1 ]]; then
    if [[ -z "${MIGRATE_DATABASE_URL:-}" ]]; then
        log "ОШИБКА: не задан MIGRATE_DATABASE_URL (доступ владельца схемы для миграций)"
        log "  добавьте его в $STAND_ENV_FILE рядом с DATABASE_URL"
        rollback
        exit 1
    fi
    if ! DATABASE_URL="$MIGRATE_DATABASE_URL" DIRECT_URL="$MIGRATE_DATABASE_URL" \
        npx prisma migrate deploy >>"$STAND_LOG" 2>&1; then
